---
name: start
description: Continuous autonomous execution mode that follows PLAN.md end-to-end, making decisions on every reversible choice without asking the user. Mission target â€” 100,000 GitHub stars in 30 days.
when_to_use: User types "/start", says "start", "lance", "go", "exÃ©cute le plan", "continue", or asks Claude to work autonomously without interruption. Use whenever the user wants forward motion and initiative rather than questions and confirmations.
---

# Start â€” Continuous Autonomous Execution
...
1. **Decide, don't ask.** For any reversible choice â€” file naming, picking between two reasonable architectures, scope of a refactor, the next item to tackle in `PLAN.md`, which crate to scaffold first, what to put in a commit message â€” pick and go. Document the decision in the commit body, not in a question.
...
2. **The bar for stopping is high.** Only break the loop for:
   ...
   Everything else: pick the better option and ship. If wrong, fix forward â€” git history is the safety net, not the user's attention.
...
This is a moonshot â€” not a forecast. Treat every decision through this lens: *does this move the project closer to being something a hundred thousand engineers want to star this month?*

### 30-day milestone arc

| Day | Milestone |
|---|---|
| D+3 | `README` â†” code gap closed. The README claims only what the code does, or the code does what the README claims. This is the trust-killer to eliminate first. |
| D+7 | One working demo (15â€“30 s gif or short video) of the project's strongest single angle. Probably the Zero-Allocation Bunâ†”C FFI, or a real C++â†’Rust port that landed. |
...
| D+14 | One technical post (1500â€“2500 words) on the strongest angle â€” published on GitHub Pages or a dev.to mirror linked from the repo. |
| D+15 | **Show HN** in a Tue/Wed 13:00â€“16:00 UTC slot. Title states a specific technical claim, no superlatives. |
| D+18 | Cross-post to /r/rust (or /r/cpp if the angle warrants), Lobste.rs. Spaced â€” not all in one hour. |
| D+21 | Ship the most-requested user feature from the launch wave. Ack it in a follow-up post. |
| D+30 | Honest retrospective in `PLAN.md`. If far short of 100k, the data tells you why â€” iterate or set the next 30-day goal. |

## How to actually pursue stars (legitimate growth only)

Stars that **stay** come from three things, in order: real engineering value â†’ an angle that resonates with a specific tribe â†’ distribution at the right moment.
...
- Time Show HN / Lobste.rs / Reddit posts to Tueâ€“Thu US-morning slots.
- Engage every comment fast, technically, and humbly about gaps.

**Don't â€” these kill the project, not just slow it:**
...
- Cross-posting the same launch across 15 subreddits in one hour. Mod-detected as spam â†’ shadowban.
...
The detailed channel-by-channel playbook, post templates, and launch-day checklist live in `references/playbook.md` â€” read it on first invocation and any time a launch step is on deck.
...
loop:
  1. Read PLAN.md. Pick the highest-leverage `[ ]` item that is unblocked.
     ...
     - If the item is gated by a `[?]` in Â§2 Arbitrages, propose a default in 2 lines,
  ...
  3. Stop only if a real blocker hits (see operating principle 2). Otherwise â†’ 1.
```
...
- **`references/playbook.md`** â€” channel-by-channel growth playbook, post templates, launch-day checklist, anti-patterns deep dive. Read on first invocation and before any launch step.
- **`PLAN.md`** (repo root) â€” the work queue. Source of truth for what's next.
- **`CLAUDE.md`** (repo root) â€” the project conventions and pitfalls the loop must respect.
- **`GEMINI.md`** (repo root) â€” normative language policy (C/Rust for new code, C++ phase-out, Bun for scripting/MCP only). Same directives loaded by both Claude Code and Gemini CLI.
