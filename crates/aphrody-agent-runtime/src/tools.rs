...
//! capabilities (shell-exec, patching, file inspection, search, and browser
//! computer use).

use std::path::PathBuf;
use std::sync::Arc;

use aphrody_agent_tools::{ApplyPatchTool, ComputerUseTool, ReadFileTool, SearchTextTool, ShellExecTool};
...
/// - `read_file` — bounded UTF-8 file inspection.
/// - `search_text` — structured repository search.
/// - `computer_use` — local browser automation when `agent-browser` is present.
///
...
pub fn build_tool_registry(config: &RuntimeConfig) -> Result<Arc<ToolRegistry>, ToolError> {
    ...
    let read_file: Arc<dyn ToolExecutor> = Arc::new(ReadFileTool::new());
    registry.register(with_default_cwd(read_file, config.cwd.as_ref()))?;

    let search_text: Arc<dyn ToolExecutor> = Arc::new(SearchTextTool::new());
    registry.register(with_default_cwd(search_text, config.cwd.as_ref()))?;

    let computer_use: Arc<dyn ToolExecutor> = Arc::new(ComputerUseTool::new());
    registry.register(with_default_cwd(computer_use, config.cwd.as_ref()))?;
