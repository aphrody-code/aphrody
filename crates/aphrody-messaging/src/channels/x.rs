...
mod tests {
    ...
    fn x_profile_deserialises_minimal() {
        let json = r#"{"id":"37252373","name":"Fixture User","username":"fixture_user"}"#;
        let p: XProfile = serde_json::from_str(json).expect("deserialise");
        assert_eq!(p.name.as_deref(), Some("Fixture User"));
        assert_eq!(p.username.as_deref(), Some("fixture_user"));
    }
