...
mod tests {
    ...
    async fn test_logging_interceptor_before() {
        ...
        let mut params = ServiceParams::new();
        let result = interceptor.before(a2a::jsonrpc::methods::SEND_MESSAGE, &mut params).await;
        assert!(result.is_ok());
    }
    ...
    async fn test_logging_interceptor_after_ok() {
        let interceptor = LoggingInterceptor;
        let result = interceptor.after(a2a::jsonrpc::methods::SEND_MESSAGE, &Ok(())).await;
        assert!(result.is_ok());
    }
    ...
    async fn test_logging_interceptor_after_err() {
        ...
        let result = interceptor.after(a2a::jsonrpc::methods::SEND_MESSAGE, &err).await;
        assert!(result.is_ok());
    }
}
