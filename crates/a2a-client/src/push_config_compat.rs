...
pub(crate) fn serialize_create_task_push_notification_config_request(
    ...
    serde_json::to_value(request).map_err(|error| {
        A2AError::internal(format!("failed to serialize create-push-config request: {error}"))
    })
}
    ...
    serde_json::from_value::<TaskPushNotificationConfig>(payload.clone()).or_else(|serde_error| {
        protojson_conv::from_value(payload).map_err(|protojson_error| {
            A2AError::internal(format!(
                "failed to deserialize push-config response: {serde_error}; ProtoJSON fallback \
                 failed: {protojson_error}"
            ))
        })
    })
}
        ...
        |serde_error| {
            serde_json::from_value::<Vec<TaskPushNotificationConfig>>(payload.clone())
                ...
                .or_else(|array_error| {
                    protojson_conv::from_value(payload).map_err(|protojson_error| {
                        A2AError::internal(format!(
                            "failed to deserialize push-config list response: {serde_error}; \
                             array fallback failed: {array_error}; ProtoJSON fallback failed: \
                             {protojson_error}"
                        ))
                    })
                })
