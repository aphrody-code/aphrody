...
use crate::{
    client::A2AClient, jsonrpc::JsonRpcTransportFactory, middleware::CallInterceptor,
    rest::RestTransportFactory, transport::TransportFactory,
};
...
impl TransportKey {
    fn from_interface(iface: &AgentInterface) -> Self {
        let major = iface
            ...
            .unwrap_or(1);
        TransportKey { protocol: iface.protocol_binding.clone(), major_version: major }
    }

    fn from_protocol(protocol: &str, version: &str) -> Self {
        let major = version.split('.').next().and_then(|s| s.parse::<u64>().ok()).unwrap_or(1);
        TransportKey { protocol: protocol.to_string(), major_version: major }
    }
}
...
impl A2AClientFactory {
    ...
    pub async fn create_from_card(
        ...
        candidates.sort_by_key(|(prio, ..)| *prio);
