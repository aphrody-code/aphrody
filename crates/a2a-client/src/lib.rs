...
//
// - wasm32-unknown-unknown (browser): JSON-RPC + REST transports via reqwest
//   browser `fetch` backend. No TLS crate needed. SSE streaming via
//   `response.bytes_stream()`. gRPC (server-side only) excluded.
//   Futures are `!Send` — BoxStream is LocalBoxStream on this target.
//
// - wasm32-wasip1: reqwest 0.13 falls back to the native HTTP stack on WASI
//   (target_os = "wasi"), which requires socket2/mio — syscalls that WASI p1
//   does not expose. The reqwest-dependent modules (jsonrpc, rest, agent_card,
//   factory) are therefore cfg-stripped on WASI. The trait definitions
//   (Transport, TransportFactory), data types, middleware, and auth ARE
//   available and compile cleanly. A future upgrade to WASI p2 / wasi-http
