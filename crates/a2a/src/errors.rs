...
impl A2AError {
    pub fn new(code: i32, message: impl Into<String>) -> Self {
        A2AError { code, message: message.into(), details: None }
    }
    ...
    pub fn task_not_found(task_id: &str) -> Self {
        A2AError::new(error_code::TASK_NOT_FOUND, format!("task not found: {task_id}"))
    }
    ...
    pub fn content_type_not_supported() -> Self {
        A2AError::new(error_code::CONTENT_TYPE_NOT_SUPPORTED, "incompatible content types")
    }
    ...
    pub fn method_not_found(method: &str) -> Self {
        A2AError::new(error_code::METHOD_NOT_FOUND, format!("method not found: {method}"))
    }
    ...
    pub fn to_jsonrpc_error(&self) -> crate::JsonRpcError {
        ...
        if let Some(details) = &self.details {
            for existing in details.iter().filter(|d| d.type_url == errordetails::ERROR_INFO_TYPE) {
                if let Some(Value::Object(meta)) = existing.value.get("metadata")
                    && let Some(Value::Object(info_meta)) = error_info.value.get_mut("metadata")
                {
                    for (k, v) in meta {
...
mod tests {
    ...
    fn test_http_status_codes() {
        ...
        assert_eq!(A2AError::content_type_not_supported().http_status_code(), 400);
        assert_eq!(A2AError::new(9999, "unknown").http_status_code(), 500);
    }

    #[test]
    fn test_http_status_codes_for_remaining_a2a_mappings() {
        assert_eq!(A2AError::push_notification_not_supported().http_status_code(), 400);
        assert_eq!(A2AError::unsupported_operation("nope").http_status_code(), 400);
        assert_eq!(A2AError::version_not_supported("9.9").http_status_code(), 400);
        assert_eq!(A2AError::parse_error("bad").http_status_code(), 400);
        assert_eq!(A2AError::invalid_request("bad").http_status_code(), 400);
        assert_eq!(A2AError::method_not_found("missing").http_status_code(), 501);
    }
    ...
    fn test_reason_to_error_code() {
        assert_eq!(reason_to_error_code("TASK_NOT_FOUND"), Some(error_code::TASK_NOT_FOUND));
        assert_eq!(
        ...
        assert_eq!(reason_to_error_code("PARSE_ERROR"), Some(error_code::PARSE_ERROR));
        assert_eq!(reason_to_error_code("INVALID_REQUEST"), Some(error_code::INVALID_REQUEST));
        assert_eq!(reason_to_error_code("METHOD_NOT_FOUND"), Some(error_code::METHOD_NOT_FOUND));
        assert_eq!(reason_to_error_code("INVALID_PARAMS"), Some(error_code::INVALID_PARAMS));
        assert_eq!(reason_to_error_code("INTERNAL_ERROR"), Some(error_code::INTERNAL_ERROR));
