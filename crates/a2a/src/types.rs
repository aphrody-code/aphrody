...
impl Part {
    ...
    pub fn raw(data: Vec<u8>) -> Self {
        Part { content: PartContent::Raw(data), filename: None, media_type: None, metadata: None }
    }
    ...
    pub fn as_text(&self) -> Option<&str> {
        if let PartContent::Text(ref s) = self.content { Some(s) } else { None }
    }
}
...
impl<'de> Deserialize<'de> for Part {
    fn deserialize<D: Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        let content = if let Some(Value::String(t)) = raw.get("text") {
            ...
            return Err(serde::de::Error::custom("Part must have one of: text, raw, url, data"));
        };

        let filename = raw.get("filename").and_then(|v| v.as_str()).map(String::from);
        let media_type = raw.get("mediaType").and_then(|v| v.as_str()).map(String::from);
        let metadata: Option<HashMap<String, Value>> =
            raw.get("metadata").and_then(|v| serde_json::from_value(v.clone()).ok());

        Ok(Part { content, filename, media_type, metadata })
    }
}
...
impl<'de> Deserialize<'de> for SendMessageResponse {
    fn deserialize<D: Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        if let Some(v) = raw.get("task") {
            ...
            Err(serde::de::Error::custom("SendMessageResponse must have 'task' or 'message'"))
        }
    }
}
...
mod tests {
    ...
    fn test_part_with_metadata_and_media_type() {
        let part = Part::text("hello").with_media_type("text/plain").with_filename("test.txt");
    ...
    fn test_subscribe_to_task_request_serde() {
        let req = SubscribeToTaskRequest { id: "t1".to_string(), tenant: None };
        let json = serde_json::to_string(&req).unwrap();
