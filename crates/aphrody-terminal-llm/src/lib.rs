...
#[cfg(feature = "mcp-default-servers")] pub use mcp::default_server_specs;
pub use mcp::{
...
pub enum LlmEvent {
    ...
    SubAgent { id: String, status: String, text: String },
    /// MCP server activity from `\e]aphrody-mcp;<server>;<state>[;<rpc>]\a`.
    Mcp { server: String, state: String, rpc: Option<String> },
    ...
    Skill { name: String, phase: String, payload: serde_json::Value },
    /// Task tree update from `\e]aphrody-task;<id>;<status>;<subject>\a`.
    Task { id: String, status: String, subject: String },
...
impl EventBus {
    ...
    pub fn publish(&self, ev: LlmEvent) -> Result<usize, broadcast::error::SendError<LlmEvent>> {
        self.tx.send(ev)
    }
