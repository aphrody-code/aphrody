...
#[test]
fn validate_skill() {
    // Walk the real shipped catalog: <repo-root>/.claude/.
    ...
    if !claude_dir.exists() {
        eprintln!(
            "skipping validate_skill: no .claude/ at {:?}",
            claude_dir
        );
        return;
    }

    let skills = load_registry(&claude_dir).expect("load shipped registry");
    assert!(
        !skills.is_empty(),
        "expected at least one shipped SKILL.md under {claude_dir:?}"
    );
