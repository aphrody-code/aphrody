...
async fn spawn_grpc_server() -> (String, tokio::task::JoinHandle<()>) {
    ...
    let handle = tokio::spawn(async move {
        Server::builder().add_service(service).serve_with_incoming(incoming).await.unwrap();
    });
...
async fn spawn_default_handler_grpc_server() -> (String, tokio::task::JoinHandle<()>) {
    let handler =
        Arc::new(DefaultRequestHandler::new(StoredTaskExecutor, InMemoryTaskStore::new()));
    let service = A2aServiceServer::new(GrpcHandler::new(handler));
    let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
    let addr = listener.local_addr().unwrap();
    let incoming = TcpListenerStream::new(listener);
...
async fn grpc_transport_end_to_end() {
    ...
    let task = transport
        .get_task(&ServiceParams::new(), &GetTaskRequest {
            id: "task-1".to_string(),
    ...
    let list = transport
        .list_tasks(&ServiceParams::new(), &ListTasksRequest {
            context_id: Some("ctx-1".to_string()),
    ...
    let canceled = transport
        .cancel_task(&ServiceParams::new(), &CancelTaskRequest {
            id: "task-1".to_string(),
            metadata: None,
            tenant: None,
        })
    ...
    let configs = transport
        .list_push_configs(&ServiceParams::new(), &ListTaskPushNotificationConfigsRequest {
            task_id: "task-1".to_string(),
            page_size: Some(10),
            page_token: Some("token-1".to_string()),
            tenant: None,
    ...
    transport
        .delete_push_config(&ServiceParams::new(), &DeleteTaskPushNotificationConfigRequest {
            task_id: "task-1".to_string(),
            id: "cfg-1".to_string(),
            tenant: None,
        })
    ...
    let card = transport
        .get_extended_agent_card(&ServiceParams::new(), &GetExtendedAgentCardRequest {
            tenant: None,
        })
        .await
        .unwrap();
...
async fn grpc_transport_streaming_and_error_paths() {
    ...
    let subscribe_stream = transport
        .subscribe_to_task(&ServiceParams::new(), &SubscribeToTaskRequest {
            id: "task-1".to_string(),
            tenant: None,
        })
        .await
...
async fn grpc_transport_accepts_bare_host_port_endpoints() {
    ...
    let factory = GrpcTransportFactory::new();
    let transport = factory
        .create(&sample_agent_card(), &AgentInterface::new(bare_endpoint, TRANSPORT_PROTOCOL_GRPC))
        .await
        .unwrap();
    let response =
        transport.send_message(&ServiceParams::new(), &send_message_request()).await.unwrap();
...
async fn grpc_transport_treats_zero_page_size_as_unset() {
    ...
    let sent = transport
        .send_message(&ServiceParams::new(), &SendMessageRequest {
