...
fn main() -> ExitCode {
    ...
    match cli.cmd {
        Cmd::Scan => match mrx::audit::run(&root, &out, &map) {
            Ok(r) => {
                tracing::info!(
                    status = ?r.status,
        ...
        Cmd::Check => match mrx::audit::run(&root, &out, &map) {
            Ok(r) if r.status == mrx::core::Status::ProductionReady => ExitCode::SUCCESS,
            Ok(r) => {
                tracing::error!(
        ...
        Cmd::Watch { debounce_ms } => match mrx::watch::run(&root, &out, &map, debounce_ms) {
            Ok(()) => ExitCode::SUCCESS,
            Err(e) => {
