...
//!   2. Detect root kind via `crate::detect::detect_root` (Bun? Turbo? pnpm?).
...
pub fn run(root: &Path, audit_out: &Path, map_out: &Path) -> Result<RunResult> {
    ...
    let root_kind = crate::detect::detect_root(&root);
    ...
    let mut workspaces: Vec<Workspace> = acc
        ...
        .filter_map(|entry| {
            ...
            let w = entry.value();
            let kind = w.kind?;
            let runtimes = crate::detect::detect_workspace_runtimes(&root.join(&path));
            Some(Workspace {
                path,
                kind,
                runtimes,
