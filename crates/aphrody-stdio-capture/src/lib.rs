...
//! # aphrody-stdio-capture
//!
//! Cross-platform in-process capture of the process's stdout AND stderr around
//! a closure, returning what was written. The redirect targets the exact
//! mechanism Rust's `std` writes through on each platform:
//!
...
//! output cannot deadlock. The redirect is process-global; callers that may run
//! concurrently must serialise around [`with_captured_stdio`] themselves.
//!
//! Used by `aphrody-ffi` (the `aphrody_run_captured` C ABI), by the cli
//! library's `run_captured`, and by the Tauri app. Distinct from the
//! `aphrody-capture` crate (Windows screen/window capture to PNG).
//!
//! On wasm32 this crate is an empty module — the mechanism is host-only.

#[cfg(not(target_arch = "wasm32"))]
pub use native::with_captured_stdio;
...
mod native {
    ...
    /// `(value, captured_stdout, captured_stderr)` where `value` is `f`'s
    /// return (typically a process exit code).
    ///
    /// Capture is best-effort: if a redirect cannot be installed the closure
    /// still runs against the normal streams and the matching captured string
    /// is empty. The closure's return value is always propagated unchanged.
    pub fn with_captured_stdio<F: FnOnce() -> i32>(f: F) -> (i32, String, String) {
        // Flush buffered output before redirecting so earlier output does not
        // leak into the captured files.
    ...
    mod unix {
        ...
        /// RAII redirect of a standard file descriptor to another file; restores
        ...
        impl Drop for Redirect {
            fn drop(&mut self) {
                // SAFETY: restore the saved descriptor over `target`, then close
                // the dup. Errors on teardown are not actionable and are ignored.
                unsafe {
    ...
    mod tests {
        ...
        fn captures_stdout_and_stderr_and_propagates_code() {
            ...
            assert_eq!(code, 7);
            assert!(out.contains("captured-out-line"), "stdout was: {out:?}");
            assert!(err.contains("captured-err-line"), "stderr was: {err:?}");
        }
