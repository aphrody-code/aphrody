...
/// Generate the full llms.txt markdown for this workspace.
/// This concatenates the contents of all markdown files.
pub fn generate_llms() -> Result<String> {
    let repo_root = repo_root()?;
    let docs_root = repo_root.join("docs");
    ...
    output.push_str("<!-- AUTO-GENERATED by aphrody-summary for AI Context -->\n\n");
    output.push_str("# Aphrody Project Context\n\n");
    
    fn append_entries(entries: &[Entry], docs_root: &Path, output: &mut String) {
        for entry in entries {
            match entry {
                Entry::File { rel_path, .. } => {
                    let full_path = docs_root.join(rel_path);
                    if let Ok(content) = fs::read_to_string(&full_path) {
                        output.push_str(&format!("## File: docs/{}\n\n", rel_path));
                        output.push_str(&content);
                        output.push_str("\n\n");
                    }
                }
                Entry::Dir { entries: inner, .. } => {
                    append_entries(inner, docs_root, output);
                }
            }
        }
    }
    
    append_entries(&entries, &docs_root, &mut output);
