...
use aphrody_design_sidecar::adapters::{ArtifactFormat, adapter_for};
use aphrody_design_sidecar::digest::{digest_artifact, digest_manifest};
use aphrody_design_sidecar::parser::Artifact;
use aphrody_design_sidecar::pipeline::Pipeline;
use aphrody_design_sidecar::validate::{validate_artifact, validate_manifest};
...
async fn run_parse(path: &str) -> Result<()> {
    ...
    let mut parser = aphrody_design_sidecar::parser::ArtifactParser::new();
    ...
    for ev in events {
        if let aphrody_design_sidecar::parser::ParserEvent::Artifact(a) = ev {
...
fn run_validate(path: &Path) -> Result<()> {
    ...
    let manifest: aphrody_design_sidecar::digest::DigestManifest =
