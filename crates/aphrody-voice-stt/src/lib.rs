...
mod tests {
    ...
    fn stt_error_missing_env_from_provider() {
        ...
        let err = crate::whisper_api::WhisperApiProvider::from_env_with_key_var(key)
    ...
    fn stt_error_missing_env_elevenlabs() {
        ...
        let err = crate::elevenlabs_stt::ElevenLabsSttProvider::from_env_with_key_var(key)
            .expect_err("should fail with missing env");
        assert!(matches!(err, SttError::MissingEnv { .. }), "unexpected error variant: {err}");
    }
    ...
    fn whisper_verbose_json_parse() {
        ...
        let transcript =
            crate::whisper_api::parse_verbose_response(&payload).expect("parse verbose response");
    ...
    fn whisper_text_response_parse() {
        let transcript = crate::whisper_api::parse_text_response("Hello world.\n");
        assert_eq!(transcript.text, "Hello world.");
