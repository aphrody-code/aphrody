...
async fn main() -> anyhow::Result<()> {
    ...
    match args.command {
        Some(Command::Launch { port, host, proxy, user_agent, stealth, workers, allow_file_access, chrome_path }) => {
            ...
            println!("ws://{}:{}/devtools/browser", host, port);
        ...
        Some(Command::ChromePath) => {
            let path = resolve_chrome_path()?;
...
fn resolve_chrome_path() -> anyhow::Result<std::path::PathBuf> {
    ...
    // Check vendor dir relative to repo root if we are running from target/release
    let repo_root = dir.parent().and_then(|p| p.parent());
    
    let platforms = [
        "linux-x64", "linux-arm64", "darwin-x64", "darwin-arm64", "windows-x64"
    ];

    let bin_name = if cfg!(windows) { "lightpanda.exe" } else { "lightpanda" };

    if let Some(root) = repo_root {
        for platform in platforms {
            let candidate = root.join("vendor").join("lightpanda-bin").join(platform).join(bin_name);
            if candidate.exists() {
                return Ok(candidate);
            }
        }
    ...
    // Try relative to cwd
    for platform in platforms {
        let candidate = std::env::current_dir()?.join("vendor").join("lightpanda-bin").join(platform).join(bin_name);
        if candidate.exists() {
            return Ok(candidate);
        }
    }
...
async fn run_multi_worker_serve(
    ...
    for i in 0..workers {
        ...
        cmd.arg("launch").arg("--port").arg(worker_port.to_string());
