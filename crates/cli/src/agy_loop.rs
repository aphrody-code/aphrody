...
fn save_state(ws: &Path, st: &LoopState) -> miette::Result<()> {
    ...
    if let Some(parent) = path.parent() {
        std::fs::create_dir_all(parent)
            .map_err(|e| miette::miette!("create .agents dir: {e}"))?;
    }
    let body =
...
fn directive(st: &LoopState) -> String {
    format!(
        "Tu n'as PAS terminé (itération {iter}/{max} de la boucle aphrody). \
         OBJECTIF : {goal}\n\n\
         Continue en autonomie totale, sans poser de question : implémente la \
         prochaine étape concrète avec du vrai code (zéro stub, zéro TODO), \
         lance le build et les tests, corrige les échecs. Si tout est déjà \
         implémenté, durcis les tests et la doc. Quand — et seulement quand — \
         l'objectif est entièrement implémenté, vérifié vert (build + tests) et \
         committé, écris le jeton exact `{done}` seul sur sa dernière ligne. \
         Sinon, reprends le travail maintenant.",
        iter = st.iteration,
