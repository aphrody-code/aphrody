...
//   * `oc-onboard`   — bootstrap state + workspace + seed config (openclaw:
//     src/commands/onboard.ts, ~110 LOC + helpers)
//   * `oc-reset`     — reset local state (3 scopes) (openclaw: src/commands/reset.ts, 151 LOC)
//   * `oc-uninstall` — clean removal (4 scopes) (openclaw: src/commands/uninstall.ts, 212 LOC)
//   * `oc-pairing`   — list/approve DM pairing requests (JSON store) (openclaw:
//     src/cli/pairing-cli.ts, 219 LOC)
//   * `oc-docs`      — open / search docs URLs (preserves the doc-link UX) (openclaw:
//     src/commands/docs.ts, 209 LOC)
//
...
fn remove_path(target: &Path, dry_run: bool) -> miette::Result<bool> {
    ...
    match result {
        ...
        Err(err) => Err(miette::miette!("Failed to remove {}: {err}", target.display())),
    }
}
...
impl TerminalCommand for OnboardCommand {
    async fn execute(&self, _ctx: &GoogleContext) -> miette::Result<()> {
        ...
        let state = state_dir()?;
        let cfg = config_path()?;
        let workspace = self.workspace.clone().map(Ok).unwrap_or_else(default_workspace_dir)?;
        ...
        if already_bootstrapped {
            println!(
                "Note: config at {} kept (pass --force to overwrite); workspace files seeded \
                 where missing.",
                cfg.display()
            );
        }
...
impl TerminalCommand for ResetCommand {
    async fn execute(&self, _ctx: &GoogleContext) -> miette::Result<()> {
        if !self.yes && !self.dry_run {
            return Err(miette::miette!(
                "Refusing to reset without --yes (or --dry-run for preview)."
            ));
        }
        let state = state_dir()?;
        let cfg = config_path()?;
...
impl TerminalCommand for UninstallCommand {
    async fn execute(&self, _ctx: &GoogleContext) -> miette::Result<()> {
        ...
        if !service && !state_flag && !workspace_flag && !app_flag {
            return Err(miette::miette!(
                "Uninstall requires explicit scopes. Use --all, or any of --service / --state / \
                 --workspace / --app."
            ));
        }
        if !self.yes && !self.dry_run {
...
impl PairingStore {
    ...
    fn approve(&mut self, channel: &str, code: &str) -> Option<PairingRequest> {
        let idx = self.pending.iter().position(|r| r.channel == channel && r.code == code)?;
...
pub(crate) enum PairingAction {
    ...
    Add { channel: String, id: String, code: String },
}
    ...
    async fn execute(&self, _ctx: &GoogleContext) -> miette::Result<()> {
        ...
        match &self.action {
            ...
            PairingAction::Approve { channel, code } => {
                let approved = store.approve(channel, code).ok_or_else(|| {
                    miette::miette!(
                        "No pending pairing request for code `{code}` on channel `{channel}`."
                    )
                })?;
                store.save(&path)?;
...
fn try_open_browser(url: &str) {
    ...
    let res: Result<std::process::Child, std::io::Error> =
        Err(io::Error::new(io::ErrorKind::Unsupported, "no browser opener for this platform"));
...
mod tests {
    ...
    async fn docs_url_only_prints_url_with_query() {
        ...
        DocsCommand { query: vec!["hello".to_string(), "world".to_string()], url_only: true }
            .execute(&ctx)
            .await
            .unwrap();
    }
