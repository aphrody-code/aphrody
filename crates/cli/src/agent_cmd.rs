...
//!
//! 1. **X / Twitter channel** — `hermes-agent` ships Telegram/Discord/Slack/
//!    WhatsApp/Signal/Email but **no X**; aphrody drives X via
//!    [`aphrody_messaging::channels::x`] (cookie-auth `aphrody-x` binary).
//! 2. **Voice-to-voice (full duplex)** — `hermes-agent` only *transcribes* voice
//!    memos (one-way). aphrody closes the loop: inbound audio → STT
//!    ([`aphrody_voice::stt`]) → chat → TTS ([`aphrody_voice`]) → outbound audio.
//! 3. **Gemini 3.5 Flash** — the chat turn runs on Gemini 3.5 Flash (the
//!    `aphrody-chat` default), keyless via the agy / web transports.
//!
//! ## Architecture
//!
...
///
/// Pipeline:
/// 1. **Resolve input** — if `cfg.voice` and the message carries an `audio/*`
///    attachment with a transcriber, fetch+transcribe it; otherwise use
///    `msg.text`.
/// 2. **Trigger gate** — skip if `cfg.trigger` is non-empty and absent from the
///    resolved text.
/// 3. **Chat** — run one turn through `chat`.
/// 4. **Reply** — `sink.reply_text`; and when `cfg.voice` + `speaker`, also
///    synthesise and `sink.reply_audio` (full voice-to-voice).
///
...
pub(crate) async fn http_fetch_audio(url: String) -> Result<Vec<u8>, AgentError> {
    ...
    resp.bytes()
        .await
        .map(|b| b.to_vec())
        .map_err(|e| AgentError::Fetch(format!("body: {e}")))
}
...
pub(crate) async fn run(
    ...
    if let Some(text) = simulate {
        ...
        eprintln!(
            "[hermes] simulate done (skipped={}, voiced={})",
            turn.skipped, turn.voiced
        );
        return Ok(());
    }
...
mod tests {
    ...
    async fn text_only_path_replies_with_text() {
        ...
        let turn = process_inbound(&msg("hello", vec![]), &mut chat, None, None, &sink, &cfg, no_fetch)
            .await
            .unwrap();
    ...
    async fn trigger_gate_skips_unmatched() {
        ...
        let turn = process_inbound(&msg("just chatter", vec![]), &mut chat, None, None, &sink, &cfg, no_fetch)
            .await
            .unwrap();
        assert!(turn.skipped);
    ...
    async fn empty_text_is_skipped() {
        ...
        let sink = RecSink::default();
        let cfg = AgentConfig::default();
        let turn = process_inbound(&msg("   ", vec![]), &mut chat, None, None, &sink, &cfg, no_fetch)
            .await
            .unwrap();
        assert!(turn.skipped);
        assert!(sink.texts.lock().unwrap().is_empty());
    ...
    async fn voice_off_ignores_audio_attachment() {
        ...
        let turn = process_inbound(&m, &mut chat, Some(&stt), None, &sink, &cfg, no_fetch)
            .await
            .unwrap();
        assert!(!turn.from_audio);
