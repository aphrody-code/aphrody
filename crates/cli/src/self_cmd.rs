    ...
    async fn execute(&self, _ctx: &GoogleContext) -> miette::Result<()> {
        ...
        if !bin.exists() {
            if self.build_if_missing {
                if self.dry_run {
                    println!("[dry-run] would run: cargo build -p aphrody --release --locked");
                } else {
                    println!(
...
fn platform_install_path(bin: &Path, dry_run: bool) -> miette::Result<()> {
    ...
    let (current_path, current_type) = unsafe {
        ...
        let rc = RegOpenKeyExW(HKEY_CURRENT_USER, subkey.as_ptr(), 0, KEY_READ, &mut hkey);
        if rc != ERROR_SUCCESS {
            return Err(miette::miette!("RegOpenKeyExW(HKCU\\Environment) failed with code {rc}"));
        }
        ...
        let (path, type_out) = if buf_size == 0 {
            ...
            if rc2 != ERROR_SUCCESS {
                ...
                return Err(miette::miette!("RegQueryValueExW(Path) failed with code {rc2}"));
            }
    ...
    let entries: Vec<&str> = current_path.split(';').filter(|s| !s.is_empty()).collect();
    let already_present = entries.iter().any(|e| {
        // Case-insensitive comparison on Windows.
        e.to_lowercase().trim_end_matches('\\') == bin_dir_str.to_lowercase().trim_end_matches('\\')
    });
    ...
    unsafe {
        let mut hkey: HKEY = std::ptr::null_mut();
        let rc =
            RegOpenKeyExW(HKEY_CURRENT_USER, subkey.as_ptr(), 0, KEY_READ | KEY_WRITE, &mut hkey);
        if rc != ERROR_SUCCESS {
            return Err(miette::miette!(
        ...
        let _ = RegCloseKey(hkey);
        if rc2 != ERROR_SUCCESS {
            return Err(miette::miette!("RegSetValueExW(Path) failed with code {rc2}"));
        }
    }
    ...
    println!(
        "open a NEW terminal (or run: $env:Path = \
         [Environment]::GetEnvironmentVariable('Path','User')) to pick it up"
    );
    print_verify_hints();
    Ok(())
}
...
fn platform_install_path(bin: &Path, dry_run: bool) -> miette::Result<()> {
    let home =
        std::env::var("HOME").into_diagnostic().wrap_err("HOME environment variable is not set")?;
    let link_dir = PathBuf::from(&home).join(".local").join("bin");
    let link = link_dir.join("aphrody");

    if dry_run {
        println!("[dry-run] would create directory {} (if missing)", link_dir.display());
        println!("[dry-run] would symlink {} -> {}", link.display(), bin.display());
    } else {
        ...
        #[cfg(unix)]
        std::os::unix::fs::symlink(bin, &link).into_diagnostic().wrap_err_with(|| {
            format!("failed to symlink {} -> {}", link.display(), bin.display())
        })?;
...
impl TerminalCommand for BootstrapCommand {
    async fn execute(&self, _ctx: &GoogleContext) -> miette::Result<()> {
        ...
        let targets =
            ["x86_64-unknown-linux-gnu", "x86_64-pc-windows-msvc", "wasm32-unknown-unknown"];
        println!();
        ...
        if !rustup_present {
            return Err(miette::miette!(
                "rustup not found on PATH — install from https://rustup.rs and re-run `aphrody \
                 self bootstrap` (use --check to inspect without installing)"
            ));
        }
        ...
        let _ =
            std::process::Command::new("rustup").args(["component", "add", "rust-src"]).status();
