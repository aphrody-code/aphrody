...
/// Behaviour:
///
/// - Reads `--config <PATH>` if supplied, otherwise calls
///   [`mcp_default_config_path`] (env / XDG / `%APPDATA%` / `./.mcp.json`).
/// - For each entry: spawns / connects, runs `initialize` + `tools/list`,
///   then emits one NDJSON line of shape
///   `{ "server": "<name>", "transport": "stdio|http|streamable-http", "tools": [...] }`.
/// - Servers that fail to start are still emitted with `{ "error": "..." }`
///   so the caller can detect partial outages.
...
impl TerminalCommand for McpListCommand {
    async fn execute(&self, _ctx: &GoogleContext) -> miette::Result<()> {
        ...
        for spec in &cfg.servers {
            let line = if self.no_probe {
                describe_static(spec)
            } else {
                describe_live(spec).await
            };
            println!("{line}");
        }
        Ok(())
...
impl TerminalCommand for McpCallCommand {
    async fn execute(&self, _ctx: &GoogleContext) -> miette::Result<()> {
        ...
        let result = client
            .call_tool(&self.tool, parsed_args)
            .await
            .map_err(|e| miette::miette!("tools/call `{}` on `{}` failed: {e}", self.tool, self.server))?;
    ...
    match path {
        ...
        None => {
            ...
            load_default_mcp_config()
                .map_err(|e| miette::miette!("load default MCP config: {e}"))
        },
    }
}

async fn connect_client(transport: &McpTransportConfig) -> Result<McpClient, gemini_runtime::McpError> {
    match transport {
        McpTransportConfig::Stdio { command, args, env } => {
            let args_refs: Vec<&str> = args.iter().map(String::as_str).collect();
            McpClient::from_stdio(command, &args_refs, env).await
        },
        McpTransportConfig::Http { url, headers } | McpTransportConfig::StreamableHttp { url, headers } => {
...
mod tests {
    ...
    fn extract_bearer_returns_none_on_missing_env() {
        ...
        h.insert(
            ...
            "Bearer ${APHRODY_TEST_DEFINITELY_UNSET}".to_owned(),
        );
    ...
    fn transport_tag_matches_variants() {
        let s = McpTransportConfig::Stdio {
            command: "x".into(),
            args: vec![],
            env: HashMap::new(),
        };
        ...
        let sh = McpTransportConfig::StreamableHttp {
            url: "https://x".into(),
            headers: HashMap::new(),
        };
