...
use aphrody_re::{
    DisasmInsn, Format, STRINGS_MIN_LEN, TriageReport, extract_strings, golang::analyze_go,
    google::analyze_google, triage_bounded,
};
...
fn analyze_file(path: &Path, deep: bool, strings_limit: usize) -> Result<AutoReport, String> {
    ...
    // --- Passe deep (Ghidra) ---
    let deep_result = if deep { Some(run_ghidra(path, &mut warnings)) } else { None };
...
fn disasm_entrypoint(
    ...
    if insns.is_empty() {
        warnings.push(format!("disasm at entrypoint {ep_va:#x} yielded no valid instructions"));
        return None;
    }

    Some(insns.into_iter().map(|i| DisasmSummary { address: i.address, text: i.text }).collect())
}
        ...
        Ok(output) if output.status.success() => {
            ...
            DeepSummary {
                ...
                ghidra_output: json_out.or_else(|| {
                    Some(serde_json::Value::String(stdout.chars().take(4096).collect()))
                }),
            }
        },
        Ok(output) => {
...
fn resolve_redress_bin() -> Option<PathBuf> {
    ...
    let gopath_bins = std::env::var("GOPATH")
        .ok()
        .map(|g| {
            g.split(if cfg!(windows) { ';' } else { ':' }).map(PathBuf::from).collect::<Vec<_>>()
        })
        .unwrap_or_default();
...
pub(crate) fn run_redress(path: &Path) -> RedressSummary {
    let Some(bin) = resolve_redress_bin() else {
        return RedressSummary {
            ...
            redress_unavailable_reason: Some(
                "redress introuvable ($REDRESS_BIN / PATH / $GOPATH/bin / ~/go/bin) — installer \
                 via `go install github.com/goretk/redress@latest`"
                    .to_owned(),
            ),
            info: None,
            packages: None,
    ...
    let run = |sub: &str| -> Option<String> {
        let out = std::process::Command::new(&bin).arg(sub).arg(target.as_ref()).output().ok()?;
...
fn run_auto_batch(dir: &Path, deep: bool, json_only: bool, limit: usize) -> Result<String, String> {
