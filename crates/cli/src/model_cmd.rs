...
use aphrody_models::{
    Catalog, Downloader, Format, HardwareProfile, ModelRef, ModelStore, ModelTask,
    Progress, PullOutcome, Report, SpeedTier, accel, human_bytes,
};
...
impl OutputOpts {
    ...
    pub(crate) fn emit_report(&self, report: &Report, json: &serde_json::Value) -> miette::Result<()> {
...
pub(crate) async fn run(action: ModelAction) -> miette::Result<()> {
    match action {
        ...
        other => {
            ...
            let store =
                ModelStore::open().map_err(|e| miette::miette!("open model store: {e}"))?;
            match other {
...
fn accel_report(output: &OutputOpts) -> miette::Result<()> {
    ...
    let mut report = Report::new("Hardware profile", &["FIELD", "VALUE"])
        ...
        .with_footer(
            "`aphrody model recommend --task ocr` ranks the catalog against this profile",
        );
    report.push(vec![
        "accelerators".into(),
    ...
    for (index, gpu) in profile.gpus.iter().enumerate() {
        ...
        report.push(vec![
            ...
            format!("{} total, {} free", human_bytes(gpu.vram_bytes), human_bytes(gpu.vram_free_bytes)),
        ]);
        report.push(vec![
...
async fn recommend(
    ...
    let mut report = Report::new(
        format!("Recommended models for {task}"),
...
fn info(store: &ModelStore, spec: &str, output: &OutputOpts) -> miette::Result<()> {
    ...
    let mut report = Report::new(
        format!("Model info: {spec}"),
        &["REFERENCE", "STATE", "SIZE", "FORMAT", "HEADER"],
    )
...
fn adopt(store: &ModelStore, path: &Path, output: &OutputOpts) -> miette::Result<()> {
    let entry = store
        .adopt_local(path)
        .map_err(|e| miette::miette!("adopt `{}`: {e}", path.display()))?;
