...
fn default_ide_candidates() -> Vec<PathBuf> {
    ...
    {
        let mut candidates =
            vec![PathBuf::from("/Applications/Antigravity IDE.app/Contents/Resources")];
        if let Some(home) = dirs::home_dir() {
            candidates.push(home.join("Applications/Antigravity IDE.app/Contents/Resources"));
        }
        candidates
    }
    ...
    {
        let mut candidates =
            vec![PathBuf::from("/opt/antigravity-ide"), PathBuf::from("/usr/lib/antigravity-ide")];
        if let Some(home) = dirs::home_dir() {
...
fn resolve_language_server(root: &Path) -> Option<PathBuf> {
    let bin_dir =
        root.join("resources").join("app").join("extensions").join("antigravity").join("bin");
...
fn run_info(pretty: bool) -> miette::Result<()> {
    ...
    let info = IdeInfo {
        ...
        electron: product
            ...
            .and_then(|v| v.as_str())
            .map(str::to_owned),
        commit: product.get("commit").and_then(|v| v.as_str()).map(str::to_owned),
        date: product.get("date").and_then(|v| v.as_str()).map(str::to_owned),
        application_name: product
            .get("applicationName")
            .and_then(|v| v.as_str())
            .map(str::to_owned),
        data_folder_name: product.get("dataFolderName").and_then(|v| v.as_str()).map(str::to_owned),
        url_protocol: product.get("urlProtocol").and_then(|v| v.as_str()).map(str::to_owned),
        paths,
    };
...
fn run_extensions(pretty: bool) -> miette::Result<()> {
    ...
    for entry in read_dir.flatten() {
        ...
        let dir_name =
            ext_dir.file_name().map(|n| n.to_string_lossy().into_owned()).unwrap_or_default();

        let name = pkg.get("name").and_then(|v| v.as_str()).unwrap_or(&dir_name).to_owned();

        let version = pkg.get("version").and_then(|v| v.as_str()).unwrap_or("0.0.0").to_owned();

        let display_name = pkg.get("displayName").and_then(|v| v.as_str()).map(str::to_owned);

        let publisher = pkg.get("publisher").and_then(|v| v.as_str()).map(str::to_owned);

        let description = pkg.get("description").and_then(|v| v.as_str()).map(str::to_owned);
...
async fn run_re(pretty: bool, include_strings: bool, strings_limit: usize) -> miette::Result<()> {
    ...
    let electron_analysis = if let Some(exe) = &paths.exe {
        ...
        let strings_sample: Option<Vec<String>> = if include_strings {
            Some(aphrody_re::extract_strings(&bytes, aphrody_re::STRINGS_MIN_LEN, strings_limit))
        } else {
            None
        };
        Some(json!({
    ...
    let language_server = if let Some(ls) = &ls_path {
        let bytes = std::fs::read(ls)
            .map_err(|e| miette::miette!("failed to read language server {}: {e}", ls.display()))?;
        ...
        .map(|e| {
            ...
            if e.is_unified_state_sync {
                if let Some(raw_val) = item_json.get(&e.key).and_then(|v| v.as_str()) {
                    if let Ok(decoded) = antigravity_sdk::state_sync::decode_unified_state(raw_val)
                    {
                        obj["decoded"] = decoded;
                    }
                }
...
fn resolve_state_vscdb() -> Option<PathBuf> {
    ...
    #[cfg(not(target_os = "windows"))]
    {
        dirs::config_dir().map(|d| {
            d.join("Antigravity IDE").join("User").join("globalStorage").join("state.vscdb")
        })
    }
}
...
fn run_launch(args: Vec<String>) -> miette::Result<()> {
    ...
    let bin = paths.launcher.as_ref().or(paths.exe.as_ref()).ok_or_else(|| {
        miette::miette!("could not find the Antigravity IDE binary in {}", root.display())
    })?;
    ...
    let status =
        cmd.status().map_err(|e| miette::miette!("failed to launch {}: {e}", bin.display()))?;
