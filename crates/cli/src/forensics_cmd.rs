...
fn map(target: &std::path::Path, out: &std::path::Path) -> miette::Result<()> {
    ...
    for entry in walkdir::WalkDir::new(target).follow_links(false) {
        let entry = entry.map_err(|e| miette::miette!("forensics map: walk error: {e}"))?;
    ...
    let files: Vec<MapEntry> = entries
        ...
        .map(|entry| {
            ...
            let sha256 =
                if !secret_meta_only && size <= MAX_HASH_BYTES { hash_file(path) } else { None };
    ...
    std::fs::create_dir_all(out)
        .map_err(|e| miette::miette!("forensics map: create out dir {}: {e}", out.display()))?;
    ...
    std::fs::write(&out_file, json)
        .map_err(|e| miette::miette!("forensics map: write {}: {e}", out_file.display()))?;
...
fn sqlite_schema(db: &std::path::Path) -> miette::Result<()> {
    ...
    if !db.is_file() {
        return Err(miette::miette!("forensics sqlite: not a file: {}", db.display()));
    }
    ...
    let conn = Connection::open_with_flags(db, flags)
        .map_err(|e| miette::miette!("forensics sqlite: open {}: {e}", db.display()))?;
    ...
    let report = SqliteReport { db: db.display().to_string(), object_count: tables.len(), tables };

    let json = serde_json::to_string_pretty(&report)
        .map_err(|e| miette::miette!("forensics sqlite: JSON encode failed: {e}"))?;
    println!("{json}");
    Ok(())
}
