    ...
    let (label, model) = if let Ok(entry) = Catalog::builtin().get(target) {
        ...
        if !path.is_file() {
            return Err(miette::miette!(
                "`{target}` is neither a catalog id nor an existing file"
            ));
        }
        let loaded =
            aphrody_infer::load(&path, &config).map_err(|e| miette::miette!("{e}"))?;
        (path.display().to_string(), loaded)
    };

    let size = std::fs::metadata(&model.path).map(|m| m.len()).unwrap_or(0);

    let mut report = Report::new(format!("Probe {label}"), &["FIELD", "VALUE"]).with_summary(
        format!("Provider chain: {}", config.providers.iter().map(|p| p.as_str()).collect::<Vec<_>>().join(" -> ")),
    );
