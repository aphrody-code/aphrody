...
impl TerminalCommand for TreeCommand {
    async fn execute(&self, _ctx: &GoogleContext) -> miette::Result<()> {
        ...
        for group in &groups {
            ...
            for entry in
                std::fs::read_dir(&base).into_diagnostic().wrap_err_with(|| {
                    format!("failed to enumerate {}", base.display())
                })?
            {
    ...
    async fn execute(&self, _ctx: &GoogleContext) -> miette::Result<()> {
        ...
        for entry in walker.flatten() {
            ...
            if DEFAULT_EXCLUDES.iter().any(|ex| {
                path_str
                    .contains(&format!("{}{}{}", std::path::MAIN_SEPARATOR, ex, std::path::MAIN_SEPARATOR))
        ...
        let mut by_type: Vec<Value> = counts
            .into_iter()
    ...
    if file_name == "bun.lock" {
        ...
        match serde_json::from_str::<Value>(&cleaned) {
            ...
            Err(e) => {
                ...
                meta.insert(
                    "parseError".into(),
                    json!(msg.chars().take(120).collect::<String>()),
                );
            },
        }
