...
pub(crate) enum AntigravityAction {
    ...
    /// 'SELECT key, value FROM ItemTable'` piped through `jq '[.[] | {key: .key, value: .value}] | add'`).
    ///
    ...
    match action {
        ...
        AntigravityAction::Onboard { tier, project, json } => {
            ...
            let response = client
                .onboard_user(&req)
                .await
                .map_err(|e| map_sdk_err("onboard", e))?;
        ...
        AntigravityAction::Refresh { json } => {
            ...
            client
                .refresh_token()
                .await
                .map_err(|e| map_sdk_err("refresh", e))?;
...
mod tests {
    ...
    fn state_sync_decode_embedded_json() {
        ...
        let summary =
            antigravity_sdk::state_sync::decode_unified_state(b64).expect("decode");
    ...
    fn item_table_parse_does_not_leak_values() {
        ...
        assert!(
            ...
            "value bytes must not appear in item-table output"
        );
    ...
    fn token_info_summary_shape() {
        let tok = make_token(
            "ya29.ABCDEFGHIJKLMNOP_long_token",
            Some("1//some_refresh"),
            Some("3599"),
        );
