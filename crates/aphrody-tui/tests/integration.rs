...
use aphrody_tui::{
    App, Render, TICK_MS, TestBackend, TuiEvent, Update, build_tick_interval,
    layout::{hsplit, vsplit},
...
fn layout_engine_splits_correctly() {
    ...
    let cols = hsplit(area, &[Constraint::Percentage(25), Constraint::Percentage(75)]);
...
fn render_loop_60fps_tick_interval() {
    ...
    assert!(ticks_per_second >= 60, "{ticks_per_second} ticks/s does not meet the 60fps contract");
    ...
    terminal
        .draw(|frame: &mut Frame<'_>| {
            let bar = McpStatusBar::new(vec![
                McpServerSlot { name: "context7".to_owned(), health: McpHealth::Up },
                McpServerSlot { name: "bxc".to_owned(), health: McpHealth::Down },
            ]);
