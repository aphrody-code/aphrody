...
//!
//! - [`ModelClient`] — an `async_trait` object-safe trait every backend
//!   (Gemini REST, gemini-web, agy) implements.
//! - [`ModelStreamEvent`] — a unified streaming event (text delta, reasoning
//!   delta, tool call, completion with token usage) that hides the wire format
//!   of the underlying API.
//! - Neutral request types ([`ChatTurn`], [`ChatMessage`], [`Part`], …) that
//!   describe a conversation turn without leaking any vendor schema.
//! - [`GeminiClient`] — a concrete client that maps Google's
//!   `:streamGenerateContent?alt=sse` wire onto the unified events.
//!
...
/// - `contents[{role, parts}]` mapping [`Part`] variants to `text` /
///   `functionCall{name,args}` / `functionResponse{name,response}`;
/// - `tools[{function_declarations:[{name,description,parameters}]}]` when any
///   tool is declared.
#[must_use]
...
///   [`ModelStreamEvent::ToolCall`] (id generated locally);
/// - `candidates[0].finishReason` and/or `usageMetadata` ->
...
mod tests {
    ...
    fn build_stream_url_trims_trailing_slash() {
        ...
        assert_eq!(
            url,
            "https://proxy.local/v1beta/models/m:streamGenerateContent?alt=sse&key=k"
        );
    }
}
