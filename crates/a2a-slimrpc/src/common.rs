...
pub fn service_params_to_metadata(params: &ServiceParamsMap) -> HashMap<String, String> {
    params.iter().map(|(key, values)| (key.clone(), values.join(", "))).collect()
}
...
fn is_internal_metadata_key(key: &str) -> bool {
    matches!(key, DEADLINE_KEY | STATUS_CODE_KEY | RPC_ID_KEY | SERVICE_KEY | METHOD_KEY)
}
...
mod tests {
    ...
    fn test_service_params_to_metadata() {
        ...
        params.insert("x-multi".to_string(), vec!["one".to_string(), "two".to_string()]);
