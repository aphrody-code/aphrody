...
use a2a_client::{
    Transport,
    transport::{ServiceParams, TransportFactory},
};
...
fn provider_config(id: &str) -> IdentityProviderConfig {
    IdentityProviderConfig::SharedSecret { id: id.to_string(), data: SHARED_SECRET.to_string() }
}

fn verifier_config(id: &str) -> IdentityVerifierConfig {
    IdentityVerifierConfig::SharedSecret { id: id.to_string(), data: SHARED_SECRET.to_string() }
}
...
impl TestEnv {
    async fn new(test_name: &str) -> Self {
        ...
        let server_name =
            Arc::new(Name::new("org".to_string(), "test".to_string(), test_name.to_string()));
        ...
        Self { test_name: test_name.to_string(), server, _server_app: server_app, server_name }
    }
...
impl RequestHandler for TestHandler {
    async fn send_message(
        ...
        let task_id = req.message.task_id.clone().unwrap_or_else(|| "task-1".to_string());

        Ok(SendMessageResponse::Task(sample_task(&task_id, TaskState::Completed)))
    }
...
async fn slimrpc_transport_end_to_end() {
    ...
    let handler = Arc::new(TestHandler { target: env.target() });
    SlimRpcHandler::new(handler).register(&env.server);
    env.start().await;

    let transport = env.transport("direct").await;
    let mut params = ServiceParams::new();
    params.insert("x-trace".to_string(), vec!["alpha".to_string(), "beta".to_string()]);

    let send_resp =
        transport.send_message(&params, &send_message_request("metadata-required")).await.unwrap();
    ...
    let task = transport
        .get_task(
            &ServiceParams::new(),
            &GetTaskRequest { id: "task-1".to_string(), history_length: Some(1), tenant: None },
        )
        .await
        .unwrap();
    ...
    let canceled = transport
        .cancel_task(
            &ServiceParams::new(),
            &CancelTaskRequest { id: "task-1".to_string(), metadata: None, tenant: None },
        )
        .await
        .unwrap();
...
async fn slimrpc_transport_streaming_and_factory_paths() {
    ...
    let subscribe_stream = transport
        .subscribe_to_task(
            &ServiceParams::new(),
            &SubscribeToTaskRequest { id: "task-1".to_string(), tenant: None },
        )
        .await
        .unwrap();
    ...
    for error in [
        transport
            .get_task(
                &ServiceParams::new(),
                &GetTaskRequest { id: "missing".to_string(), history_length: None, tenant: None },
            )
            .await
            .unwrap_err(),
        transport
            .cancel_task(
                &ServiceParams::new(),
                &CancelTaskRequest { id: "missing".to_string(), metadata: None, tenant: None },
            )
            .await
            .unwrap_err(),
        transport
    ...
    let subscribe_stream = transport
        .subscribe_to_task(
            &ServiceParams::new(),
            &SubscribeToTaskRequest { id: "missing".to_string(), tenant: None },
        )
        .await
        .unwrap();
    let subscribe_events: Vec<_> = subscribe_stream.collect().await;
    assert_eq!(subscribe_events.len(), 1);
    assert_eq!(subscribe_events[0].as_ref().unwrap_err().code, error_code::TASK_NOT_FOUND,);
...
async fn slimrpc_transport_reports_malformed_payloads() {
    ...
    let error = transport
        .get_task(
            &ServiceParams::new(),
            &GetTaskRequest { id: "task-1".to_string(), history_length: None, tenant: None },
        )
        .await
        .unwrap_err();
