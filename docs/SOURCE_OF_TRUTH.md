...
- **Web/UI** : la surface UI est le monorepo **Material Design 3** (Bun + TS,
  libs `@aphrody-code/*` dans `packages/*`). Le cœur Rust
...
| Build VPS / Rust | Toolchain épinglée par `rust-toolchain.toml` ; `CARGO_TARGET_DIR=~/aphrody/target/linux-gnu` ; sous Linux, passer `--config .cargo/config.linux-vps.toml --config "build.rustc-wrapper=''"` à Cargo. |
