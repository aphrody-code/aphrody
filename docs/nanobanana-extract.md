...
- **Attribution**: preserved in `aphrody/aphrody/images.py` header comment
...
Add these to `aphrody/pyproject.toml` (under `[project] dependencies`):
...
CLI wrapper (to be wired in `aphrody/aphrody/cli/main.py`):
```
