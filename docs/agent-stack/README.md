...
### Grok Build optionnel : `~/.grok/config.toml`

Le fichier peut subsister même lorsque le binaire `grok` n'est pas installé.
Toujours vérifier `command -v grok` avant les commandes ci-dessous.
...
### Headless `grok -p` (snapshot, si installé)

```bash
grok -p "task" --always-approve --permission-mode bypassPermissions --max-turns 80
```

Models: `grok-build` (default), `grok-composer-2.5-fast` (`-m`). **Do not** pass `--effort` or `--reasoning-effort` with `grok-build` (HTTP 400).
...
## Diagnostics et smoke optionnel

```bash
aphrody doctor
bash ~/aphrody/scripts/test-xai-grok-bxc.sh  # GET /models, aucun chat par défaut
# Opt-in potentiellement facturable : RUN_PAID_XAI_SMOKE=1 bash ~/aphrody/scripts/test-xai-grok-bxc.sh
