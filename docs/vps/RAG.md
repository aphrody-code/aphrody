...
## 1. RAG Souverain (Obsolète — Supprimé)

> [!IMPORTANT]
> L'inférence LLM locale (`llama.cpp` / `rpbey-llm.service`) et le sidecar d'embeddings local (`rpbey-embed.service`) ont été entièrement supprimés du VPS en juin 2026 afin de libérer la mémoire vive (RAM) et optimiser l'utilisation du processeur.
>
> Les fonctionnalités d'inférence, de chat et de génération s'appuient désormais de manière unifiée sur des APIs distantes (comme l'API Google Gemini via `gemini-2.5-flash` ou le binaire d'agent `aphrody`).
