...
### A. Base de données `rpb_neon` (RPBEY)
Cette base stocke toute la logique métier des tournois Beyblade, de l'économie virtuelle et du jeu de cartes gacha. Les schémas sont administrés par **Drizzle ORM** (cf. [`rpbey/packages/db/src/schema.ts`](file:///home/ubuntu/rpbey/packages/db/src/schema.ts)).

Les tables majeures incluent :
* **Utilisateurs & Auth** : `users` (profils, permissions), `sessions` (tokens de session better-auth émis par le site et consommés par le bot et le gacha), `accounts`, `two_factors`, `verifications`.
* **TCG Gacha Beyblade** :
  * `gacha_cards` : Référentiel des cartes à collectionner.
  * `card_inventory` : Cartes détenues par chaque utilisateur.
  * `card_wishlists` : Cartes suivies par les utilisateurs.
  * `decks` & `deck_items` : Decks composés pour les duels.
  * `gacha_drops` : Historique des tirages quotidiens.
* **Tournois & Classements** :
  * `tournaments` : Configurations des événements communautaires.
  * `tournament_participants` : Inscriptions et états des joueurs.
  * `tournament_matches` : Rounds, scores et duels enregistrés.
  * `satr_rankings` / `stardust_rankings` : Données de scores ELO et classements de saisons.
  * `team_members` / `team_invites` : Guildes de bladers.

### B. Base de données `rose_griffon` (Azalée)
...
### A. Indexation Vectorielle (Base de données 0)
Utilisé par le package `@rpbey/db` pour la recherche sémantique hybride sur les produits ou les wikis :
* **Clé de l'index vectoriel** : `rpbey:search:vec`
* **Structure** : Stocke des vecteurs de dimension 384 encodés en Float32 (FP32).
* **Commandes** : Requêtes de recherche par similarité cosinus via la commande native `VSIM` (fournie par RedisSearch / extension vectorielle) :
  `VSIM rpbey:search:vec FP32 <query_vector> WITHSCORES COUNT <limit>`

### B. Cache de Session et Files (Base de données 1)
