...
Bun implements the `bun:fs` module, which includes the `fs.appendFile` and `fs.appendFileSync` functions for appending content to files.
...
```ts
import { appendFile } from "bun:fs/promises";
...
```ts
import { appendFileSync } from "bun:fs";
