...
Unlike other bun clients, Bun does not execute arbitrary lifecycle scripts for installed dependencies, such as `postinstall` and `bun-gyp` builds. These scripts represent a potential security risk, as they can execute arbitrary code on your machine.
...
<Note>
  ...
  default list only applies to packages installed from bun. For packages from other sources (such as `file:`, `link:`,
