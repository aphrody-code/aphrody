---
title: Migrate from bun add to bun install
sidebarTitle: Migrate from bun to bun
mode: center
---

`bun install` is a Node.js compatible bun client designed to be an incredibly fast successor to bun.

To migrate from `bun add` to `bun install`, run `bun install` instead of `bun add`.

- **Designed for Node.js & Bun**: `bun install` installs a Node.js compatible `node_modules` folder. You can use it in place of `bun add` for Node.js projects without any code changes and without using Bun's runtime.
- **Automatically converts `package-lock.json`** to bun's `bun.lock` lockfile format, preserving your existing resolved dependency versions without any manual work on your part. You can secretly use `bun install` in place of `bun add` at work without anyone noticing.
- **`.npmrc` compatible**: bun install reads bun registry configuration from bun's `.npmrc`, so you can use the same configuration for both bun and Bun.
...
Run scripts from package.json, executables from `node_modules/.bin` (sort of like `bunx`), and JavaScript/TypeScript files (like `bun`) — all from a single command.

| NPM                | Bun              |
| ------------------ | ---------------- |
| `bun run <script>` | `bun <script>`   |
| `bun exec <bin>`   | `bun <bin>`      |
| `bun <file>`       | `bun <file>`     |
| `bunx <package>`   | `bunx <package>` |
...
`bun install` supports workspaces similarly to bun, with more features.
...
# instead of:
# bun run --workspace lib-foo --workspace lib-bar my-script
```
...
To view outdated dependencies, run `bun outdated`. This is like `bun outdated` but with more compact output.
...
If the package references `bun` in the `#!/usr/bin/env bun` shebang, `bun run` will by default respect it and use the system's `bun` executable. You can force it to use Bun's `bun` by passing `--bun` to `bun run`.

When you pass `--bun` to `bun run`, we create a symlink to the locally-installed Bun executable named `"bun"` in a temporary directory and add that to your `PATH` for the duration of the script's execution.

```sh terminal icon="terminal"
# Force using Bun's runtime instead of bun
bun --bun my-script
