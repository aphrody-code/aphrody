...
Bun implements the `bun:fs` module, including the `fs.watch` function for listening for file system changes.
...
Using the `bun:fs/promises` module, you can listen for changes using `for await...of` instead of a callback.
