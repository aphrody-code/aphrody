...
The `--bun` flag tells Bun to run Vite's CLI using `bun` instead of `bun`; by default Bun respects Vite's `#!/usr/bin/env bun` [shebang line](<https://en.wikipedia.org/wiki/Shebang_(Unix)>).
