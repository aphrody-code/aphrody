...
You can run the test suite either using `bun test <path>` or by using the wrapper script `bun bun:test <path>`. The `bun bun:test` command runs every test file in a separate instance of bun.exe, to prevent a crash in the test runner from stopping the entire suite.
