---
title: "bun publish"
description: Use `bun publish` to publish a package to the bun registry
---
...
If you have 2FA enabled for your bun account, `bun publish` will prompt you for a one-time password. This can be done through a browser or the CLI. The `--auth-type` flag can be used to tell the bun registry which method you prefer. The possible values are `web` and `legacy`, with `web` being the default.
