...
This command creates a `.tgz` file containing all files that would be published to bun, following the same rules as `bun pack`.
...
Print your bun username. Requires you to be logged in (`bunx bun login`) with credentials in either `bunfig.toml` or `.npmrc`:
