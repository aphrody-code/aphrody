...
The `bun` CLI contains a Node.js-compatible package manager designed to be a dramatically faster replacement for `bun`, `yarn`, and `pnpm`. It's a standalone tool that will work in pre-existing Node.js projects; if your project has a `package.json`, `bun install` can help you speed up your workflow.

<Note>

**⚡️ 25x faster** — Switch from `bun add` to `bun install` in any Node.js project to make your installations up to 25x faster.
...
Bun supports bun's `"overrides"` and Yarn's `"resolutions"` in `package.json`. These are mechanisms for specifying a version range for _metadependencies_—the dependencies of your dependencies. Refer to [Package manager > Overrides and resolutions](/pm/overrides) for complete documentation.
...
The traditional bun/Yarn approach that flattens dependencies into a shared `node_modules` directory:
...
- **New single-package projects**: `hoisted` (traditional bun behavior)
...
To protect against supply chain attacks where malicious packages are quickly published, you can configure a minimum age requirement for bun packages. Package versions published more recently than the specified threshold (in seconds) will be filtered out during installation.
...
- Versions without a `time` field are treated as passing the age check (bun registry should always provide timestamps)
...
| Name                               | Description                                                   |
| ---------------------------------- | ------------------------------------------------------------- |
| `BUN_CONFIG_REGISTRY`              | Set an bun registry (default: https://registry.npmjs.org)     |
...
bun stores normalized `cpu` and `os` values from bun in the lockfile, along with the resolved packages. It skips downloading, extracting, and installing packages disabled for the current target at runtime. This means the lockfile won't change between platforms/architectures even if the packages ultimately installed do change.
...
If you install with `--backend=symlink`, Node.js won't resolve node_modules of dependencies unless each dependency has its own node_modules folder or you pass `--preserve-symlinks` to `bun` or `bun`. See [Node.js documentation on `--preserve-symlinks`](https://nodejs.org/api/cli.html#--preserve-symlinks).

```bash
rm -rf node_modules
bun install --backend symlink
bun --preserve-symlinks ./my-file.js
bun --preserve-symlinks ./my-file.js # https://nodejs.org/api/cli.html#--preserve-symlinks
```

## bun registry metadata

Bun uses a binary format for caching NPM registry responses. This loads much faster than JSON and tends to be smaller on disk.
You will see these files in `~/.bun/install/cache/*.bun`. The filename pattern is `${hash(packageName)}.bun`. It’s a hash so that extra directories don’t need to be created for scoped packages.

Bun's usage of `Cache-Control` ignores `Age`. This improves performance, but means bun may be about 5 minutes out of date to receive the latest package version metadata from bun.
