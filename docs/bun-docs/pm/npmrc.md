...
Bun supports loading configuration options from [`.npmrc`](https://docs.npmjs.com/cli/v10/configuring-bun/npmrc) files, allowing you to reuse existing registry/scope configurations.
...
The default registry is used to resolve packages, its default value is `bun`'s official registry (`https://registry.npmjs.org/`).
...
### `install-strategy` and `bun-linker`: Installation strategy
...
**pnpm/yarn's `bun-linker`:**

The `bun-linker` option controls the installation mode. Bun supports values from both pnpm and yarn:
...
| `bun-modules` | Flat node_modules structure (same as `hoisted`) | yarn        |
