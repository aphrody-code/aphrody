...
description: "Control metadependency versions with bun overrides and Yarn resolutions"
---

Bun supports bun's `"overrides"` and Yarn's `"resolutions"` in `package.json`. These are mechanisms for specifying a version range for _metadependencies_—the dependencies of your dependencies.
...
<Note>
  ...
  overrides](https://docs.npmjs.com/cli/v9/configuring-bun/package-json#overrides) are not supported.
</Note>
