...
import { readFileSync, promises as fs } from "bun:fs";
...
You can embed `.bun` files into executables.

```ts index.ts icon="/icons/typescript.svg"
const addon = require("./addon.bun");

console.log(addon.hello());
```

Unfortunately, if you're using `@mapbox/bun-pre-gyp` or other similar tools, you'll need to make sure the `.bun` file is directly required or it won't bundle correctly.
