...
When shipping a library containing a macro to bun or another package registry, use the `"macro"` export condition to provide a special version of your package exclusively for the macro environment.
...
When Bun's transpiler sees a macro import, it calls the function inside the transpiler using Bun's JavaScript runtime and converts the return value from JavaScript into an AST bun. These JavaScript functions are called at bundle-time, not runtime.
