...
`.js` `.cjs` `.mjs` `.mts` `.cts` `.ts` `.tsx` `.jsx` `.css` `.json` `.jsonc` `.json5` `.toml` `.yaml` `.yml` `.txt` `.wasm` `.bun` `.html` `.sh`
...
**Native addon loader**. Default for `.bun`.

In the runtime, native addons can be directly imported.

```ts
import addon from "./addon.bun";
console.log(addon);
```

In the bundler, `.bun` files are handled using the [`file`](#file) loader.
