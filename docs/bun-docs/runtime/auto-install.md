...
3. Otherwise, download and install the appropriate version from the `bun` registry.
...
import { z } from "zod@next"; // bun tag
...
- **Convenience** — There's no need to run `bun add` or `bun install` before running a file or script. Run it with `bun run`.
...
<AccordionGroup>
    ...
    Deno requires an `bun:` specifier before each bun `import`, lacks support for import maps via `compilerOptions.paths` in `tsconfig.json`, and has incomplete support for `package.json` settings. Unlike Deno, Bun does not currently support URL imports.
    </Accordion>
