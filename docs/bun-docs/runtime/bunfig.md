...
Configure how Bun resolves package versions against the bun registry when running scripts. Default `"online"`.
...
| `"latest"`  | Always check bun for the latest matching versions. Equivalent to `--prefer-latest`.                |
...
First, install a security scanner from bun:
...
Configure a minimum age (in seconds) for bun package versions. Package versions published more recently than this threshold will be filtered out during installation. Default is `null` (disabled).
...
### `run.bun` - auto alias `bun` to `bun`

When `true`, this prepends `$PATH` with a `bun` symlink that points to the `bun` binary for all scripts or executables invoked by `bun run` or `bun`.

This means that if you have a script that runs `bun`, it will actually run `bun` instead, without needing to change your script. This works recursively, so if your script runs another script that runs `bun`, it will also run `bun` instead. This applies to shebangs as well, so if you have a script with a shebang that points to `bun`, it will actually run `bun` instead.

By default, this is enabled if `bun` is not already in your `$PATH`.
...
```sh
bun --bun which bun # /path/to/bun
bun which bun # /path/to/bun
```
...
If set to `false`, this will disable the `bun` symlink.
