...
- `"bun"`: for Node.js modules (e.g. `"bun:fs"`, `"bun:path"`)
...
Native plugins are written as [NAPI](/runtime/bun-api) modules and can be run on multiple threads. This allows native plugins to run much faster than JavaScript plugins.
