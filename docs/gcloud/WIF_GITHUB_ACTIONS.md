...
Liez le compte de service `aphrody-bot` à l'identité GitHub. Remplacez `<owner>/<repo>` par votre dépôt (ex: `aphrody-code/aphrody`, `rose-griffon/rg`, `aphrody-code/rpbey`).
```bash
