...
- [~] **T3.1** Typed M3 panels: `doctor` dashboard, version/system, the high-value command surfaces (re / forensics / chat / image / ...) as views; streaming output via Channels. **Done (2026-05-26):** `doctor --json` → typed M3 dashboard (verdict chip + runtime / peer-A2A / supply-chain / workspace / environment cards), `version --json` → typed version/system card, `re triage --pretty` → typed panel (format/sha256 header + sections table with Shannon-entropy bars + imports/exports + strings sample). `network` → typed DNS-recon panel (counted, filterable subdomain grid) + web-search result cards (opener-guarded links); `forensics` → typed `sqlite` schema panel (objects grouped by type, collapsible CREATE) + `map` summary panel. `chat` is already the rich assistant view. All parse the exact Rust JSON/text schema with defensive fallback; `bun run build` green. **Remaining:** `image` (Nano Banana) as a typed view and streaming output via Tauri Channels — both need a Tauri-bridge change (an image-bytes command since the bridge exposes no arbitrary file read; a streaming command since `aphrody_exec`/`run_captured` is one-shot global-stdio capture), i.e. a distinct frontend+backend effort, not pure UI.
