...
> Mise à jour 2026-09-05 : la cible produit est désormais le serveur Codex
> self-hosted d'Aphrody. Le chemin agent par défaut est local et sans clé via
> `LocalResponsesClient` (`APHRODY_LOCAL_MODEL_URL`, `/v1/responses`) ; Gemini
> reste une référence historique de ce plan, pas une exigence d'exécution.

La première façade app-server est désormais livrée : `aphrody session serve`
parle JSON-RPC NDJSON sur stdin/stdout et expose `initialize`, `thread/start`,
`thread/list`, `turn/start` et `shutdown`. Elle applique le handshake Codex :
`initialize`, puis la notification `initialized`, avant les opérations de
thread. Elle reste locale et annonce explicitement qu'aucune authentification
n'est requise.

Le transport expose aussi un chemin de streaming local :
`turn/start` accepte `waitForCompletion:false` et `turn/events` retourne les
événements disponibles sans attendre la fin du modèle. Le mode synchrone reste
