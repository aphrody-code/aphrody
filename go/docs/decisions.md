...
**Note:** This is part of a series of documents that outline [Go Style](index.md)at Google. This document is **[normative](index.md#normative) but not[canonical](index.md#canonical)**, and is subordinate to the[core style guide](guide.md). See [the overview](index.md#about) for more information.
...
This document is **not exhaustive** and will grow over time. In cases where[the core style guide](guide.md) contradicts the advice given here, **the style
...
in all lowercase (e.g., [`tabwriter`](https://pkg.go.dev/text/tabwriter) instead of `tabWriter`, `TabWriter`, or`tab_writer`).

Avoid selecting package names that are likely to be [shadowed](best-practices.md#shadowing) by commonly used
...
* Using the `_test` suffix for unit tests that only exercise the exported API
  of a package (package `testing` calls these[“black box tests”](https://pkg.go.dev/testing)). For example, a package`linkedlist` must define its black box unit tests in a package named`linkedlist_test` (not `linked_list_test`)
...
Constant names must use [MixedCaps](guide.md#mixed-caps) like all other names in Go. ([Exported](https://tour.golang.org/basics/3)constants start with uppercase, while unexported constants start with
...
have the same case. `URL` should appear as `URL` or `url` (as in `urlPony`, or`URLPony`), never as `Url`. As a general rule, identifiers (e.g., `ID` and `DB`)
...
judgement based on context, [clarity](guide.md#clarity), and [concision](guide.md#concision).
...
Single-letter variable names can be a useful tool to minimize[repetition](#repetition), but can also make code needlessly opaque. Limit their
...
experience of reading the documentation of a public API. See[Effective Go](http://golang.org/doc/effective_go.html#commentary) for more
information.

The best practices document’s section on [documentation conventions](best-practices.md#documentation-conventions) discusses
this further.

**Best Practice:** Use [doc preview](best-practices.md#documentation-preview) during development and code review to see
...
There is no fixed [line length](guide.md#line-length) for comments in Go.
...
Packages should clearly document their intended usage. Try to provide a[runnable example](http://blog.golang.org/examples); examples show up in Godoc. Runnable examples belong in the
...
name result parameters just because it enables you to use naked returns.[Clarity](guide.md#clarity) is always more important than saving a few lines in
your function.
...
Local names for imported packages must follow[the guidance around package naming](#package-names), including the prohibition
on the use of underscores and capital letters. Try to be[consistent](guide.md#consistency) by always using the same local name for the same
imported package.

An imported package *must* be renamed to avoid a name collision with other
imports. (A corollary of this is that [good package names](#package-names)should not require renaming.) In the event of a name collision, prefer to rename
the most local or project-specific import.

Generated protocol buffer packages *must* be renamed to remove underscores from
their names, and their local names must have a `pb` suffix. See[proto and stub best practices](best-practices.md#import-protos) for more
information.
...
can get wrapped into an interface and thus become a non-nil value (see the[Go FAQ entry on the topic](https://golang.org/doc/faq#nil_error)).
...
Go’s support for multiple return values provides a better solution (see the[Effective Go section on multiple returns](http://golang.org/doc/effective_go.html#multiple-returns)). Instead of requiring clients to
...
ensure that the [braces match](#literal-matching-braces) to avoid[indentation confusion](#indentation-confusion).
...
these cases a stack trace will not help the reader. Please note that[`log.Exit`](https://pkg.go.dev/github.com/golang/glog#Exit) calls [`os.Exit`](https://pkg.go.dev/os#Exit) and any deferred functions will not be run.
...
exclusively at[package initialization time](https://golang.org/ref/spec#Package_initialization)(e.g. [template.Must](https://golang.org/pkg/text/template/#Must) and[regexp.MustCompile](https://golang.org/pkg/regexp/#MustCompile)).
...
Where `Must` functions are used in a test, they should generally be[marked as a test helper](#mark-test-helpers) and call `t.Fatal` on error (see[error handling in test helpers](best-practices.md#test-helper-error-handling) for
more considerations of using that).

They should not be used when[ordinary error handling](best-practices.md#error-handling) is possible (including
...
the scope of a function and factoring out the logic into[synchronous functions](#synchronous-functions). If the concurrency is still not obvious, it is
...
* Do not wrap RPC clients in new manual interfaces just for the sake of
  abstraction or testing.[Use real transports](best-practices.md#use-real-transports) instead
...
constructs like command, chaining, factory, and[strategy](https://en.wikipedia.org/wiki/Strategy_pattern) patterns.

Deeper discussion on interfaces exists in the[Best Practices’ section on interfaces](best-practices.md#interfaces).
...
burden on readers. Instead prefer established [error handling](#errors)practices. For testing, be especially wary of introducing[assertion libraries](#assert) or frameworks that result in less useful[test failures](#useful-test-failures).
...
were a regular function parameter. The choice between the two is based on which[method set(s)](https://golang.org/ref/spec#Method_sets) the method should be a part of.
...
* If the receiver is a struct containing fields that[cannot safely be copied](#copying), use a pointer receiver. Common examples
...
* If the receiver is a “large” struct or array, a pointer receiver may be more
  ...
  elements as arguments to the method. If that seems too large to[pass by value](#pass-values), a pointer is a good choice.
...
Do not use `break` statements without target labels at the ends of `switch`clauses; they are redundant. Unlike in C and Java, `switch` clauses in Go
...
Use a *type definition*, `type T1 T2`, to define a new type. Use a[*type alias*](http://golang.org/ref/spec#Type_declarations), `type T1 = T2`, to refer to an existing type without defining a
...
Go programs in the Google codebase use an internal variant of the[standard `flag` package](https://golang.org/pkg/flag/). It has a similar interface but interoperates well
...
should follow the standard Go name style ([mixed caps](guide.md#mixed-caps)). Specifically, the flag
...
There is additional discussion around best practices for creating [complex CLIs](best-practices.md#complex-clis)with subcommands.
...
Go programs in the Google codebase use a variant of the standard [`log`](https://pkg.go.dev/log)package. It has a similar but more powerful interface and interoperates well
...
* Best practices on [logging errors](best-practices.md#error-logging) and[custom verbosity levels](best-practices.md#vlog)
* When and how to use the log package to[stop the program](best-practices.md#checks-and-panics)
...
* In streaming RPC methods, where the context comes from the stream.
  ...
  the generated server type, which implements `grpc.ServerStream`. See[gRPC Generated Code documentation](https://grpc.io/docs/languages/go/generated-code/).

* In test functions (e.g. `TestXXX`, `BenchmarkXXX`, `FuzzXXX`), where the
  context comes from[`(testing.TB).Context()`](https://pkg.go.dev/testing#TB.Context).
...
Do not create custom context types or use interfaces other than[`context.Context`](https://pkg.go.dev/context) in function signatures. There are no exceptions to this
rule.
...
Assertion libraries tend to either stop the test early (if `assert` calls`t.Fatalf` or `panic`) or omit relevant information about what the test got
right:
...
* For more on the distinction between test helpers and assertion helpers, see[best practices](best-practices.md#test-functions)
...
include a key to aid in interpreting the failure. See the[section on printing diffs](#print-diffs). Whichever diff order you use in your failure
...
expecting your function to return, and compare directly using a[deep comparison](#types-of-equality).
...
(e.g., if one of the fields is an `io.Reader`), tweaking a [`cmp.Diff`](https://pkg.go.dev/github.com/google/go-cmp/cmp#Diff) or[`cmp.Equal`](https://pkg.go.dev/github.com/google/go-cmp/cmp#Equal) comparison with [`cmpopts`](https://pkg.go.dev/github.com/google/go-cmp/cmp/cmpopts) options such as[`cmpopts.IgnoreInterfaces`](https://pkg.go.dev/github.com/google/go-cmp/cmp/cmpopts#IgnoreInterfaces) may meet your needs
...
meaningless or even mislead the investigator. Note how the code below calls`t.Fatalf` and *then* `t.Errorf`:
...
**Best practice:** For more discussion about when `t.Fatal` should be used, see[best practices](best-practices.md#t-fatal).
...
handled by `==`, such as slices. Use [`cmp.Equal`](https://pkg.go.dev/github.com/google/go-cmp/cmp#Equal) for equality comparison and[`cmp.Diff`](https://pkg.go.dev/github.com/google/go-cmp/cmp#Diff) to obtain a human-readable diff between objects.
...
The conventional failure message, which is suitable for most Go tests, is`YourFunc(%v) = %v, want %v`. However, there are cases that may call for more or
less detail:
...
* Setup failures do not require the same level of detail. If a test helper
  ...
  include which test input you were going to store in the database.`t.Fatalf("Setup: Failed to set up test database: %s", err)` is usually
...
future changes. For guidance on constructing errors with semantic meaning see[best-practices regarding errors](best-practices.md#error-handling). If an error
...
some other error, then consider using [`errors.Is`](https://pkg.go.dev/errors#Is) or `cmp` with[`cmpopts.EquateErrors`](https://pkg.go.dev/github.com/google/go-cmp/cmp/cmpopts#EquateErrors).

>
>
> **Note:** If a test uses [`cmpopts.EquateErrors`](https://pkg.go.dev/github.com/google/go-cmp/cmp/cmpopts#EquateErrors) but all of its `wantErr`values are either `nil` or `cmpopts.AnyError`, then using `cmp` is[unnecessary mechanism](guide.md#least-mechanism). Simplify the code by making
...
using them is not mandatory. See also the[Go blog post about subtests](https://blog.golang.org/subtests).
...
cases, it is appropriate to write multiple test functions, as explained in[GoTip #50: Disjoint Table Tests](https://google.github.io/styleguide/go/index.html#gotip).
...
always [identify the input](#identify-the-input). The table test row names must follow the[subtest naming](#subtest-names) guidance.
...
and the conditions that led to it. Specifically, the guidance about[assert libraries](#assert) still applies, and [`t.Helper`](https://pkg.go.dev/testing#T.Helper) should not be used
...
This may enable better test coverage and more concise tests. Be aware that any[examples](#examples) declared in the test will not have the package names that a user will
need in their code.
...
package as the code being tested. In these cases, use a package name with the`_test` suffix. This is an exception to the “no underscores” rule to[package names](#package-names). For example:
...
testing framework permitted for Go code in the Google codebase. In particular,[assertion libraries](#assert) and third-party testing frameworks are not
allowed.
...
These are designed to work cohesively with core language features like[composite literal](https://go.dev/ref/spec#Composite_literals) and [if-with-initializer](https://go.dev/ref/spec#If_statements) syntax to enable test authors to
...
* **Empty composite literal vs. `new` or `make`**. `&File{}` and `new(File)`are equivalent. So are `map[string]bool{}` and `make(map[string]bool)`. See
