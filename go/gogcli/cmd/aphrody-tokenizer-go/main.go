...
import (
	...
	gogcmd "github.com/steipete/gogcli/internal/cmd"
	_ "github.com/steipete/gogcli/internal/tzembed"
...
type Request struct {
	...
	// Google Cloud Platform API fields
...
// PubSubMessage defines the JSON format for a pulled message.
...
type Response struct {
	...
	// Google Cloud Platform API response fields
...
func main() {
	// Auto-configure GOG_AUTH_MODE=adc if GOOGLE_APPLICATION_CREDENTIALS is set
	if os.Getenv("GOG_AUTH_MODE") == "" && os.Getenv("GOOGLE_APPLICATION_CREDENTIALS") != "" {
		os.Setenv("GOG_AUTH_MODE", "adc")
	}
	...
	if len(os.Args) >= 2 {
		if os.Args[1] == "--help" || os.Args[1] == "-h" {
			...
			fmt.Println("  aphrody-tokenizer-go upload_asset <local_path> [display_name]    Upload a file/asset to the configured Drive folder")
			fmt.Println("  aphrody-tokenizer-go gcs_upload <bucket> <object> <local_file> [content_type] Upload a file to Google Cloud Storage")
			fmt.Println("  aphrody-tokenizer-go gcs_download <bucket> <object> <local_file>              Download a file from Google Cloud Storage")
			fmt.Println("  aphrody-tokenizer-go pubsub_publish <project_id> <topic> <text>               Publish a message to Google Pub/Sub")
			fmt.Println("  aphrody-tokenizer-go pubsub_pull <project_id> <subscription> [max_messages]   Pull messages from Google Pub/Sub")
			fmt.Println("  aphrody-tokenizer-go bq_query <project_id> <sql_query>                        Run a BigQuery query")
			fmt.Println("  aphrody-tokenizer-go secret_get <project_id> <secret_name> [version]          Retrieve secret from Secret Manager")
			fmt.Println("  aphrody-tokenizer-go firestore_crud <project_id> <path> [content_json]        Read/write Firestore collection/document")
			fmt.Println("  aphrody-tokenizer-go kms_encrypt <key_path> <plaintext>                       Encrypt text using KMS key")
			fmt.Println("  aphrody-tokenizer-go kms_decrypt <key_path> <ciphertext_base64>               Decrypt text using KMS key")
			fmt.Println("  aphrody-tokenizer-go tasks_create <project_id> <location> <queue> <url> [body] Create task in Cloud Tasks queue")
			fmt.Println("  aphrody-tokenizer-go translate <project_id> <text> <target_lang>              Translate text using Translation API")
			fmt.Println("  aphrody-tokenizer-go speech_to_text <project_id> <location> <gcs_uri>         Transcribe audio file from GCS")
			fmt.Println("  aphrody-tokenizer-go vision_annotate <gcs_uri>                                Detect labels in GCS image using Vision API")
			fmt.Println("  aphrody-tokenizer-go vision_safesearch <gcs_uri>                              Run safe-search detection on GCS image")
			fmt.Println("  aphrody-tokenizer-go tts_synthesize <text> <out_path> [voice] [lang]          Synthesize audio file from text using TTS")
			fmt.Println("  aphrody-tokenizer-go logging_write <project_id> <log_name> <msg> [severity]   Write entry to Cloud Logging")
			fmt.Println("  aphrody-tokenizer-go logging_list <project_id> <log_name> [max] [filter]      Retrieve Cloud Logging entries")
			fmt.Println("  aphrody-tokenizer-go cloudrun_list <project_id> [region]                      List Cloud Run services")
			fmt.Println("  aphrody-tokenizer-go cloudfunctions_list <project_id> [region]                List Cloud Functions")
			fmt.Println("  aphrody-tokenizer-go spanner_query <database_path> <sql>                      Run Spanner SQL query")
			fmt.Println("  aphrody-tokenizer-go spanner_write <database_path> <dml_stmt>                 Execute Spanner DML statement")
			fmt.Println("  aphrody-tokenizer-go gmail_clean <account_email> [max_threads] [--dry-run]    Clean Gmail inbox using Gemini 2.5")
		...
		if os.Args[1] == "upload_asset" {
			if len(os.Args) < 3 {
				fmt.Fprintln(os.Stderr, "Error: local path is required. Usage: upload_asset <local_path> [display_name]")
				os.Exit(1)
			}
			localPath := os.Args[2]
			...
			err := handleUploadAssetCLI(localPath, displayName)
			if err != nil {
				fmt.Fprintf(os.Stderr, "Error: %v\n", err)
				os.Exit(1)
			}
		...
		if os.Args[1] == "gcs_upload" {
			if len(os.Args) < 5 {
				fmt.Fprintln(os.Stderr, "Error: bucket, object and local_file are required. Usage: gcs_upload <bucket> <object> <local_file> [content_type]")
				os.Exit(1)
			}
			ctx := context.Background()
			req := Request{
		...
		if os.Args[1] == "gcs_download" {
			if len(os.Args) < 5 {
				fmt.Fprintln(os.Stderr, "Error: bucket, object and local_file are required. Usage: gcs_download <bucket> <object> <local_file>")
				os.Exit(1)
			}
			ctx := context.Background()
			req := Request{
			...
			resp, err := runGCSDownload(ctx, req)
			if err != nil {
				fmt.Fprintf(os.Stderr, "Error: %v\n", err)
				os.Exit(1)
			}
		...
		if os.Args[1] == "pubsub_publish" {
			if len(os.Args) < 5 {
				fmt.Fprintln(os.Stderr, "Error: project_id, topic and text are required. Usage: pubsub_publish <project_id> <topic> <text>")
				os.Exit(1)
			}
			ctx := context.Background()
			req := Request{
			...
			resp, err := runPubSubPublish(ctx, req)
			if err != nil {
				fmt.Fprintf(os.Stderr, "Error: %v\n", err)
				os.Exit(1)
			}
		...
		if os.Args[1] == "pubsub_pull" {
			if len(os.Args) < 4 {
				fmt.Fprintln(os.Stderr, "Error: project_id and subscription are required. Usage: pubsub_pull <project_id> <subscription> [max_messages]")
				os.Exit(1)
			}
			ctx := context.Background()
			req := Request{
				ProjectID:    os.Args[2],
				Subscription: os.Args[3],
			}
			if len(os.Args) >= 5 {
				val, _ := strconv.Atoi(os.Args[4])
				req.MaxMessages = val
			}
			resp, err := runPubSubPull(ctx, req)
			if err != nil {
				fmt.Fprintf(os.Stderr, "Error: %v\n", err)
				os.Exit(1)
			}
		...
		if os.Args[1] == "bq_query" {
			if len(os.Args) < 4 {
				fmt.Fprintln(os.Stderr, "Error: project_id and sql_query are required. Usage: bq_query <project_id> <sql_query>")
				os.Exit(1)
			}
			ctx := context.Background()
			req := Request{
		...
		if os.Args[1] == "secret_get" {
			if len(os.Args) < 4 {
				fmt.Fprintln(os.Stderr, "Error: project_id and secret_name are required. Usage: secret_get <project_id> <secret_name> [version]")
				os.Exit(1)
			}
			ctx := context.Background()
			req := Request{
			...
			if len(os.Args) >= 5 {
				req.SecretVersion = os.Args[4]
			}
			resp, err := runSecretGet(ctx, req)
			if err != nil {
				fmt.Fprintf(os.Stderr, "Error: %v\n", err)
				os.Exit(1)
			}
		...
		if os.Args[1] == "firestore_crud" {
			if len(os.Args) < 4 {
				fmt.Fprintln(os.Stderr, "Error: project_id and path are required. Usage: firestore_crud <project_id> <path> [content_json]")
				os.Exit(1)
			}
			ctx := context.Background()
			req := Request{
			...
			if len(os.Args) >= 5 {
				req.Content = os.Args[4]
			}
			resp, err := runFirestoreCRUD(ctx, req)
			if err != nil {
				fmt.Fprintf(os.Stderr, "Error: %v\n", err)
				os.Exit(1)
			}
		...
		if os.Args[1] == "kms_encrypt" {
			if len(os.Args) < 4 {
				fmt.Fprintln(os.Stderr, "Error: key_path and plaintext are required. Usage: kms_encrypt <key_path> <plaintext>")
				os.Exit(1)
			}
			ctx := context.Background()
			req := Request{
			...
			resp, err := runKMSEncrypt(ctx, req)
			if err != nil {
				fmt.Fprintf(os.Stderr, "Error: %v\n", err)
				os.Exit(1)
			}
		...
		if os.Args[1] == "kms_decrypt" {
			if len(os.Args) < 4 {
				fmt.Fprintln(os.Stderr, "Error: key_path and ciphertext are required. Usage: kms_decrypt <key_path> <ciphertext_base64>")
				os.Exit(1)
			}
			ctx := context.Background()
			req := Request{
			...
			resp, err := runKMSDecrypt(ctx, req)
			if err != nil {
				fmt.Fprintf(os.Stderr, "Error: %v\n", err)
				os.Exit(1)
			}
		...
		if os.Args[1] == "tasks_create" {
			if len(os.Args) < 6 {
				fmt.Fprintln(os.Stderr, "Error: project_id, location, queue and url are required. Usage: tasks_create <project_id> <location> <queue> <url> [body]")
				os.Exit(1)
			}
			ctx := context.Background()
			req := Request{
			...
			if len(os.Args) >= 7 {
				req.Content = os.Args[6]
			}
			resp, err := runCloudTasksCreate(ctx, req)
			if err != nil {
				fmt.Fprintf(os.Stderr, "Error: %v\n", err)
				os.Exit(1)
			}
		...
		if os.Args[1] == "translate" {
			if len(os.Args) < 5 {
				fmt.Fprintln(os.Stderr, "Error: project_id, text and target_lang are required. Usage: translate <project_id> <text> <target_lang>")
				os.Exit(1)
			}
			ctx := context.Background()
			req := Request{
		...
		if os.Args[1] == "speech_to_text" {
			if len(os.Args) < 5 {
				fmt.Fprintln(os.Stderr, "Error: project_id, location and gcs_uri are required. Usage: speech_to_text <project_id> <location> <gcs_uri>")
				os.Exit(1)
			}
			ctx := context.Background()
			req := Request{
			...
			resp, err := runSpeechToText(ctx, req)
			if err != nil {
				fmt.Fprintf(os.Stderr, "Error: %v\n", err)
				os.Exit(1)
			}
		...
		if os.Args[1] == "vision_annotate" {
			if len(os.Args) < 3 {
				fmt.Fprintln(os.Stderr, "Error: gcs_uri is required. Usage: vision_annotate <gcs_uri>")
				os.Exit(1)
			}
			ctx := context.Background()
			req := Request{
			...
			resp, err := runVisionAnnotate(ctx, req)
			if err != nil {
				fmt.Fprintf(os.Stderr, "Error: %v\n", err)
				os.Exit(1)
			}
		...
		if os.Args[1] == "vision_safesearch" {
			if len(os.Args) < 3 {
				fmt.Fprintln(os.Stderr, "Error: gcs_uri is required. Usage: vision_safesearch <gcs_uri>")
				os.Exit(1)
			}
			ctx := context.Background()
			req := Request{
			...
			resp, err := runVisionSafeSearch(ctx, req)
			if err != nil {
				fmt.Fprintf(os.Stderr, "Error: %v\n", err)
				os.Exit(1)
			}
		...
		if os.Args[1] == "tts_synthesize" {
			if len(os.Args) < 4 {
				fmt.Fprintln(os.Stderr, "Error: text and out_path are required. Usage: tts_synthesize <text> <out_path> [voice] [lang]")
				os.Exit(1)
			}
			ctx := context.Background()
			req := Request{
			...
			if len(os.Args) >= 6 {
				req.Language = os.Args[5]
			}
			resp, err := runTextToSpeech(ctx, req)
			if err != nil {
				fmt.Fprintf(os.Stderr, "Error: %v\n", err)
				os.Exit(1)
			}
		...
		if os.Args[1] == "logging_write" {
			if len(os.Args) < 5 {
				fmt.Fprintln(os.Stderr, "Error: project_id, log_name and msg are required. Usage: logging_write <project_id> <log_name> <msg> [severity]")
				os.Exit(1)
			}
			ctx := context.Background()
			req := Request{
			...
			if len(os.Args) >= 6 {
				req.Severity = os.Args[5]
			}
			resp, err := runLoggingWrite(ctx, req)
			if err != nil {
				fmt.Fprintf(os.Stderr, "Error: %v\n", err)
				os.Exit(1)
			}
		...
		if os.Args[1] == "logging_list" {
			if len(os.Args) < 4 {
				fmt.Fprintln(os.Stderr, "Error: project_id and log_name are required. Usage: logging_list <project_id> <log_name> [max] [filter]")
				os.Exit(1)
			}
			ctx := context.Background()
			req := Request{
			...
			if len(os.Args) >= 6 {
				req.Filter = os.Args[5]
			}
			resp, err := runLoggingList(ctx, req)
			if err != nil {
				fmt.Fprintf(os.Stderr, "Error: %v\n", err)
				os.Exit(1)
			}
		...
		if os.Args[1] == "cloudrun_list" {
			if len(os.Args) < 3 {
				fmt.Fprintln(os.Stderr, "Error: project_id is required. Usage: cloudrun_list <project_id> [region]")
				os.Exit(1)
			}
			ctx := context.Background()
			req := Request{
			...
			if len(os.Args) >= 4 {
				req.Location = os.Args[3]
			}
			resp, err := runCloudRunList(ctx, req)
			if err != nil {
				fmt.Fprintf(os.Stderr, "Error: %v\n", err)
				os.Exit(1)
			}
		...
		if os.Args[1] == "cloudfunctions_list" {
			if len(os.Args) < 3 {
				fmt.Fprintln(os.Stderr, "Error: project_id is required. Usage: cloudfunctions_list <project_id> [region]")
				os.Exit(1)
			}
			ctx := context.Background()
			req := Request{
			...
			resp, err := runCloudFunctionsList(ctx, req)
			if err != nil {
				fmt.Fprintf(os.Stderr, "Error: %v\n", err)
				os.Exit(1)
			}
		...
		if os.Args[1] == "spanner_query" {
			if len(os.Args) < 4 {
				fmt.Fprintln(os.Stderr, "Error: database_path and sql are required. Usage: spanner_query <database_path> <sql>")
				os.Exit(1)
			}
			ctx := context.Background()
			req := Request{
			...
			resp, err := runSpannerQuery(ctx, req)
			if err != nil {
				fmt.Fprintf(os.Stderr, "Error: %v\n", err)
				os.Exit(1)
			}
		...
		if os.Args[1] == "gmail_clean" {
			if len(os.Args) < 3 {
				fmt.Fprintln(os.Stderr, "Error: account_email is required. Usage: gmail_clean <account_email> [max_threads] [--dry-run]")
				os.Exit(1)
			}
			accountEmail := os.Args[2]
			maxThreads := 50
			dryRun := false
			...
			for i := 3; i < len(os.Args); i++ {
				...
				if arg == "--dry-run" {
					...
					var val int
					if _, err := fmt.Sscan(arg, &val); err == nil && val > 0 {
						maxThreads = val
					}
				}
			}
			...
			req := Request{
				...
				MaxMessages:  maxThreads,
				DryRun:       dryRun,
			}
			resp, err := runGmailClean(ctx, req)
			if err != nil {
				fmt.Fprintf(os.Stderr, "Error: %v\n", err)
				os.Exit(1)
			}
		...
		// Fallback: Delegate unrecognized commands to the Google Workspace CLI (gogcli)
		if err := gogcmd.Execute(os.Args[1:]); err != nil {
			os.Exit(gogcmd.ExitCode(err))
		}
		os.Exit(0)
	}
	...
	// Google Cloud Platform Command Router
	if req.Command == "gcs_upload" {
		resp, err := runGCSUpload(ctx, req)
		if err != nil {
			writeJSONError("gcs_upload failed: " + err.Error())
			os.Exit(1)
		}
		_ = json.NewEncoder(os.Stdout).Encode(resp)
		os.Exit(0)
	...
	if req.Command == "gcs_download" {
		resp, err := runGCSDownload(ctx, req)
		if err != nil {
			writeJSONError("gcs_download failed: " + err.Error())
			os.Exit(1)
		}
		_ = json.NewEncoder(os.Stdout).Encode(resp)
		os.Exit(0)
	...
	if req.Command == "pubsub_publish" {
		resp, err := runPubSubPublish(ctx, req)
		if err != nil {
			writeJSONError("pubsub_publish failed: " + err.Error())
			os.Exit(1)
		}
		_ = json.NewEncoder(os.Stdout).Encode(resp)
		os.Exit(0)
	...
	if req.Command == "pubsub_pull" {
		resp, err := runPubSubPull(ctx, req)
		if err != nil {
			writeJSONError("pubsub_pull failed: " + err.Error())
			os.Exit(1)
		}
		_ = json.NewEncoder(os.Stdout).Encode(resp)
		os.Exit(0)
	...
	if req.Command == "bq_query" {
		resp, err := runBQQuery(ctx, req)
		if err != nil {
			writeJSONError("bq_query failed: " + err.Error())
			os.Exit(1)
		}
		_ = json.NewEncoder(os.Stdout).Encode(resp)
		os.Exit(0)
	...
	if req.Command == "secret_get" {
		resp, err := runSecretGet(ctx, req)
		if err != nil {
			writeJSONError("secret_get failed: " + err.Error())
			os.Exit(1)
		}
		_ = json.NewEncoder(os.Stdout).Encode(resp)
		os.Exit(0)
	...
	if req.Command == "firestore_crud" {
		resp, err := runFirestoreCRUD(ctx, req)
		if err != nil {
			writeJSONError("firestore_crud failed: " + err.Error())
			os.Exit(1)
		}
		_ = json.NewEncoder(os.Stdout).Encode(resp)
		os.Exit(0)
	...
	if req.Command == "kms_encrypt" {
		resp, err := runKMSEncrypt(ctx, req)
		if err != nil {
			writeJSONError("kms_encrypt failed: " + err.Error())
			os.Exit(1)
		}
		_ = json.NewEncoder(os.Stdout).Encode(resp)
		os.Exit(0)
	...
	if req.Command == "kms_decrypt" {
		resp, err := runKMSDecrypt(ctx, req)
		if err != nil {
			writeJSONError("kms_decrypt failed: " + err.Error())
			os.Exit(1)
		}
		_ = json.NewEncoder(os.Stdout).Encode(resp)
		os.Exit(0)
	...
	if req.Command == "tasks_create" {
		resp, err := runCloudTasksCreate(ctx, req)
		if err != nil {
			writeJSONError("tasks_create failed: " + err.Error())
			os.Exit(1)
		}
		_ = json.NewEncoder(os.Stdout).Encode(resp)
		os.Exit(0)
	...
	if req.Command == "translate" {
		resp, err := runTranslate(ctx, req)
		if err != nil {
			writeJSONError("translate failed: " + err.Error())
			os.Exit(1)
		}
		_ = json.NewEncoder(os.Stdout).Encode(resp)
		os.Exit(0)
	...
	if req.Command == "speech_to_text" {
		resp, err := runSpeechToText(ctx, req)
		if err != nil {
			writeJSONError("speech_to_text failed: " + err.Error())
			os.Exit(1)
		}
		_ = json.NewEncoder(os.Stdout).Encode(resp)
		os.Exit(0)
	...
	if req.Command == "vision_annotate" {
		resp, err := runVisionAnnotate(ctx, req)
		if err != nil {
			writeJSONError("vision_annotate failed: " + err.Error())
			os.Exit(1)
		}
		_ = json.NewEncoder(os.Stdout).Encode(resp)
		os.Exit(0)
	...
	if req.Command == "vision_safesearch" {
		resp, err := runVisionSafeSearch(ctx, req)
		if err != nil {
			writeJSONError("vision_safesearch failed: " + err.Error())
			os.Exit(1)
		}
		_ = json.NewEncoder(os.Stdout).Encode(resp)
		os.Exit(0)
	...
	if req.Command == "tts_synthesize" {
		resp, err := runTextToSpeech(ctx, req)
		if err != nil {
			writeJSONError("tts_synthesize failed: " + err.Error())
			os.Exit(1)
		}
		_ = json.NewEncoder(os.Stdout).Encode(resp)
		os.Exit(0)
	...
	if req.Command == "logging_write" {
		resp, err := runLoggingWrite(ctx, req)
		if err != nil {
			writeJSONError("logging_write failed: " + err.Error())
			os.Exit(1)
		}
		_ = json.NewEncoder(os.Stdout).Encode(resp)
		os.Exit(0)
	...
	if req.Command == "logging_list" {
		resp, err := runLoggingList(ctx, req)
		if err != nil {
			writeJSONError("logging_list failed: " + err.Error())
			os.Exit(1)
		}
		_ = json.NewEncoder(os.Stdout).Encode(resp)
		os.Exit(0)
	...
	if req.Command == "cloudrun_list" {
		resp, err := runCloudRunList(ctx, req)
		if err != nil {
			writeJSONError("cloudrun_list failed: " + err.Error())
			os.Exit(1)
		}
		_ = json.NewEncoder(os.Stdout).Encode(resp)
		os.Exit(0)
	...
	if req.Command == "cloudfunctions_list" {
		resp, err := runCloudFunctionsList(ctx, req)
		if err != nil {
			writeJSONError("cloudfunctions_list failed: " + err.Error())
			os.Exit(1)
		}
		_ = json.NewEncoder(os.Stdout).Encode(resp)
		os.Exit(0)
	...
	if req.Command == "spanner_query" {
		resp, err := runSpannerQuery(ctx, req)
		if err != nil {
			writeJSONError("spanner_query failed: " + err.Error())
			os.Exit(1)
		}
		_ = json.NewEncoder(os.Stdout).Encode(resp)
		os.Exit(0)
	...
	if req.Command == "spanner_write" {
		resp, err := runSpannerWrite(ctx, req)
		if err != nil {
			writeJSONError("spanner_write failed: " + err.Error())
			os.Exit(1)
		}
		_ = json.NewEncoder(os.Stdout).Encode(resp)
		os.Exit(0)
	...
	if req.Command == "gmail_clean" {
		resp, err := runGmailClean(ctx, req)
		if err != nil {
			writeJSONError("gmail_clean failed: " + err.Error())
			os.Exit(1)
		}
		_ = json.NewEncoder(os.Stdout).Encode(resp)
		os.Exit(0)
	...
	if req.Command == "upload_asset" {
		folderID := os.Getenv("GOG_DRIVE_ASSETS_FOLDER_ID")
		if folderID == "" {
			writeJSONError("GOG_DRIVE_ASSETS_FOLDER_ID is not configured in the environment")
			os.Exit(1)
		}
		if req.LocalFile == "" {
			writeJSONError("local_file is required for upload_asset command")
			os.Exit(1)
		}
		args := []string{"--json", "drive", "upload", req.LocalFile, "--parent", folderID}
		if req.DisplayName != "" {
			args = append(args, "--name", req.DisplayName)
		}
		err := gogcmd.Execute(args)
		if err != nil {
			writeJSONError("upload_asset failed: " + err.Error())
			os.Exit(1)
		}
		os.Exit(0)
	}
...
func handleUploadAssetCLI(localPath, displayName string) error {
	folderID := os.Getenv("GOG_DRIVE_ASSETS_FOLDER_ID")
	if folderID == "" {
		return fmt.Errorf("GOG_DRIVE_ASSETS_FOLDER_ID is not set in the environment.\n" +
			"Please:\n" +
			"1. Create a folder on your personal Google Drive.\n" +
			"2. Share it with 'aphrody-bot@aphrody.iam.gserviceaccount.com' as Editor.\n" +
			"3. Set GOG_DRIVE_ASSETS_FOLDER_ID in your environment or .env file.")
	}

	args := []string{"drive", "upload", localPath, "--parent", folderID}
	if displayName != "" {
		args = append(args, "--name", displayName)
	}

	return gogcmd.Execute(args)
}
