...
func TestUploadAsset_Validation(t *testing.T) {
	t.Setenv("GOG_DRIVE_ASSETS_FOLDER_ID", "")
	err := handleUploadAssetCLI("some_path", "")
	if err == nil {
		t.Error("expected error when folder ID is empty, got nil")
	} else if !strings.Contains(err.Error(), "GOG_DRIVE_ASSETS_FOLDER_ID is not set") {
		t.Errorf("unexpected error message: %v", err)
	}
}

func TestGCPCLI_Subcommands(t *testing.T) {
	binPath := "./aphrody-tokenizer-go.exe"
	if _, err := os.Stat(binPath); err != nil {
		binPath = "gogcli/cmd/aphrody-tokenizer-go/aphrody-tokenizer-go.exe"
		if _, err := os.Stat(binPath); err != nil {
			t.Skip("skipping subprocess execution test; binary not found at", binPath)
			return
		}
	}

	cmd := exec.Command(binPath, "gcs_upload", "bucket")
	var stderr bytes.Buffer
	cmd.Stderr = &stderr
	err := cmd.Run()
	if err == nil {
		t.Error("expected command to fail with missing arguments, but it succeeded")
	}

	errMsg := stderr.String()
	if !strings.Contains(errMsg, "bucket, object and local_file are required") {
		t.Errorf("expected error message to contain usage info, got: %q", errMsg)
	}
}

func TestGmailClean_Validation(t *testing.T) {
	binPath := "./aphrody-tokenizer-go.exe"
	if _, err := os.Stat(binPath); err != nil {
		binPath = "gogcli/cmd/aphrody-tokenizer-go/aphrody-tokenizer-go.exe"
		if _, err := os.Stat(binPath); err != nil {
	...
	cmd := exec.Command(binPath, "gmail_clean")
	var stderr bytes.Buffer
	cmd.Stderr = &stderr
	err := cmd.Run()
	if err == nil {
	...
	errMsg := stderr.String()
	if !strings.Contains(errMsg, "account_email is required") {
		t.Errorf("expected error message to contain usage info, got: %q", errMsg)
	}
}
