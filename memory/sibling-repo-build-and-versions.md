...
- The `rg` consumer pins `@aphrody-code/bxc: ^0.6.0` (caret) → auto-pick a
  published 0.6.x; no consumer manifest edits needed. `bxc/packages/x` exists,
  while `x-client` does not; the CLAUDE.md x-client-extraction note is stale.
