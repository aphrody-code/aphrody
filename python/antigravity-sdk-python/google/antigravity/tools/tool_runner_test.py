...
class ProcessToolCallsTest(absltest.TestCase):
    ...
    def test_multiple_tool_calls(self):
        ...
        results = asyncio.run(
            runner.process_tool_calls(
                [
                    sdk_types.ToolCall(
                        name="_sample_tool", args={"arg1": "World"}
                    ),
                    sdk_types.ToolCall(
                        name="_async_tool", args={"x": 1, "y": 2}
                    ),
                ]
            )
        )
    ...
    def test_missing_args_defaults_to_empty(self):
        ...
        results = asyncio.run(
            runner.process_tool_calls(
                [sdk_types.ToolCall(name="_no_args_tool")]
            )
        )
        self.assertLen(results, 1)
...
class ContextInjectionTest(absltest.TestCase):
    ...
    def _make_mock_context(self):
        ...
        from google.antigravity.tools import (
            tool_context,  # pylint: disable=g-import-not-at-top
        )
    ...
    def test_async_tool_with_context(self):
        ...
        async def _async_context_tool(
            x: int, ctx: tool_context.ToolContext
        ) -> int:
            nonlocal received_ctx
            received_ctx = ctx
            return x * 2
    ...
    def test_no_context_set_skips_injection(self):
        ...
        def _optional_ctx_tool(
            ...
            del (
                arg1
            )  # Unused, exists to verify injection doesn't break extra args.
