"use client";

/**
 * @license
 * Copyright 2026 Google LLC
 * SPDX-License-Identifier: Apache-2.0
 */

import * as React from "react";

import {
  BaseTransitionProps,
  isVisible,
  resolveTimeout,
  useTransitionState,
} from "./use-transition-state.js";

const ENTER_EASING =
  "var(--md-sys-motion-easing-emphasized-decelerate, cubic-bezier(0.05, 0.7, 0.1, 1))";
const EXIT_EASING =
  "var(--md-sys-motion-easing-emphasized-accelerate, cubic-bezier(0.3, 0, 0.8, 0.15))";
const ENTER_DURATION_FALLBACK = 300;
const EXIT_DURATION_FALLBACK = 200;

/** The edge the child slides in from. */
export type SlideDirection = "left" | "right" | "up" | "down";

export interface SlideProps extends BaseTransitionProps {
  /**
   * The edge the child enters from (and exits toward). Defaults to `down`,
   * i.e. the child starts below its resting position and slides up into view.
   */
  direction?: SlideDirection;
  /**
   * The off-screen offset used while hidden. Accepts any CSS length; defaults
   * to `100%` of the element's own size on the relevant axis.
   */
  offset?: string;
}

function hiddenTransform(direction: SlideDirection, offset: string): string {
  switch (direction) {
    case "left":
      return `translateX(${offset})`;
    case "right":
      return `translateX(-${offset})`;
    case "up":
      return `translateY(${offset})`;
    case "down":
    default:
      return `translateY(-${offset})`;
  }
}

/**
 * Slides its child in from a screen edge and back out, animating `transform`.
 * The MUI `Slide` equivalent.
 */
export function Slide(props: SlideProps): React.ReactElement | null {
  const {
    in: inProp,
    timeout,
    appear = false,
    mountOnEnter = false,
    unmountOnExit = false,
    direction = "down",
    offset = "100%",
    children,
  } = props;

  const enterMs = resolveTimeout(timeout, "enter", ENTER_DURATION_FALLBACK);
  const exitMs = resolveTimeout(timeout, "exit", EXIT_DURATION_FALLBACK);

  const { mounted, phase } = useTransitionState(inProp, {
    appear,
    mountOnEnter,
    unmountOnExit,
    enterTimeout: enterMs,
    exitTimeout: exitMs,
  });

  if (!mounted) {
    return null;
  }

  const visible = isVisible(phase);
  const durationMs = visible ? enterMs : exitMs;
  const easing = visible ? ENTER_EASING : EXIT_EASING;

  const childStyle = (children.props as { style?: React.CSSProperties }).style;
  const style: React.CSSProperties = {
    ...childStyle,
    transform: visible ? "translate(0, 0)" : hiddenTransform(direction, offset),
    transition: `transform ${durationMs}ms ${easing}`,
    willChange: "transform",
  };

  return React.cloneElement(children, {
    style,
  } as React.HTMLAttributes<HTMLElement>);
}
