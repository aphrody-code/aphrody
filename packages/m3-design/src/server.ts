...
if (promptIndex !== -1) {
  ...
  const designSystem = designSystemIndex !== -1 ? args[designSystemIndex + 1] : undefined;
  ...
  const PORT = 3005;
  console.log(`[HTTP Server] Starting automated design engine on http://localhost:${PORT}...`);
    ...
    async fetch(req) {
      ...
      if (req.method === "POST" && url.pathname === "/api/generate/finalize") {
        try {
          ...
          if (!projectId) {
            return Response.json({ error: "Missing projectId field" }, { status: 400 });
          }
          if (!apiKey) {
          ...
          if (protocol === "anthropic") {
            ...
            return Response.json({ error: `Unsupported protocol: ${protocol}` }, { status: 400 });
          }
