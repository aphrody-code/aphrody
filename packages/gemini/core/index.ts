...
//
// In both cases ElevenLabs (voice.ts) handles TTS. No paid API key is
...
/**
 * Run a single-turn chat against the active backend and yield the
 * assistant's reply as a stream of strings.
 */
export async function* askStream(prompt: string): AsyncGenerator<string, void, unknown> {
  const runtime = resolveRuntime();
  const messages: ChatMessage[] = [{ role: "user", content: prompt }];

  if (runtime.backend === "gemini-oauth") {
    yield* askGeminiStream(messages);
    return;
  }
  // Fallback to non-streaming for gateway for now, or just yield one chunk
  const text = await askGateway(messages);
  yield text;
}

function createGenAIClient(auth: string): GoogleGenAI {
  return new GoogleGenAI({
    apiKey: "DUMMY", // The SDK requires some string here
    httpOptions: {
      headers: {
        authorization: auth,
      },
 ...
 *
 * Uses the new @google/genai SDK with a custom fetch injector for OAuth.
 */
async function askGemini(messages: ReadonlyArray<ChatMessage>): Promise<string> {
  ...
  const ai = createGenAIClient(auth);
...
/**
 * Backend A (Streaming): Uses the new @google/genai SDK to stream the response.
 */
async function* askGeminiStream(messages: ReadonlyArray<ChatMessage>): AsyncGenerator<string, void, unknown> {
  const model = process.env["GEMINI_MODEL"] ?? "gemini-2.0-flash-exp";
  const auth = await authorizationHeader();
    ...
    case "auth": {
      if (rest[0] === "login") {
        process.stderr.write(
          "aphrody native OAuth is not yet implemented.\n" +
          "Please install the reference CLI globally via `bun install -g @google/gemini-cli` and run `gemini auth login` to generate the credentials file.\n"
        );
        return 1;
      }
      process.stderr.write(`unknown auth subcommand: ${rest[0]}\n`);
      return 2;
    }
