...
export class ListController<Item extends ListItem> {
  ...
  /**
   * Cached resolved items array. Invalidated by `onSlotchange` so that
   * repeated calls to `this.items` within a single event handler (e.g. during
   * keyboard navigation or deactivation) do not re-walk the DOM.
   */
   ...
   *
   * The result is cached and invalidated on `slotchange` events so that
   * multiple reads within a single event dispatch avoid re-walking the DOM.
   */
  get items(): Item[] {
   ...
   */
  onSlotchange = () => {
    // Invalidate the cached items array so the next read re-walks the DOM.
