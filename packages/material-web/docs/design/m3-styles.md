...
Reference digest of the Material Design 3 _Styles_ section, paraphrased in our own
...
| Icons (Material Symbols)    | _no crate module_ — `m3-tokens` does not model the Symbols axes (weight/fill/grade/optical size); also `state.rs`, `adaptive.rs`, `gemini_brand.rs`, `google_sans_*` exist beyond the core five style areas |
...
2. **Motion `emphasized` easing cubic-bezier diverges from spec.** `motion.rs`
   ...
   `EASING_STANDARD`. That matches the spec's _guidance_ (web has no single bezier for
