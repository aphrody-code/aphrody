...
2. **System tokens** (`md.sys.*`) — the _decisions and roles_ that give the
...
every consumer without touching any token _name_ — the names and their meanings
...
dark-theme context. Light/dark mode is the canonical example — the _same_ system
