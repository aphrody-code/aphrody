...
1. **Outlined / Elevated / Text button label colour role**
   ...
   buttons spec the _default_ outlined button label/icon is **On-surface-variant**
...
4. **FAB default container role naming** (`actions.rs:189`, `:227`).
   ...
   baseline _secondary_-container tone, not primary-container
...
7. **Checkbox container size** (`inputs.rs:99`).
   ...
   broader spec target/state-layer is 40dp and the _icon container_ is commonly
