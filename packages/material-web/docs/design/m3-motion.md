...
_emphasized_ easing set with longer durations; small utility transitions use the
_standard_ set with short durations. All values are published as
`md.sys.motion.*` system tokens.

## Easing tokens

Two sets, three curves each. Values are the official CSS `cubic-bezier(...)`
forms (control points). Note the **emphasized (full)** curve is a _non-cubic_
path on Android/Flutter; CSS/iOS have **no single-bézier equivalent** and the
spec says to fall back to _standard_ for that one token.
...
| --------------------- | -------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| emphasized            | `EASING_EMPHASIZED` = `cubic-bezier(0.2, 0.0, 0, 1.0)`   | ✅ present (matches the spec's recommended _standard_ fallback; spec lists CSS as N/A) |
...
| medium3     | 350 ms     | _(crate calls 350 `LONG1`)_ | ⚠️ **mislabeled** — should be `medium3` |
| medium4     | 400 ms     | _(crate calls 400 `LONG2`)_ | ⚠️ **mislabeled** — should be `medium4` |
