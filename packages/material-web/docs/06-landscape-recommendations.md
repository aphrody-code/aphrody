---
title: "Panorama & Recommandations"
nav_order: 8
---
...
| Option                                                                       | Nature                                       | Framework                   | Santé 2026                                                            |
| ---------------------------------------------------------------------------- | -------------------------------------------- | --------------------------- | --------------------------------------------------------------------- |
| **`@material/web` (Material Web Components, MWC)**                           | Web components officiels Google (Lit)        | Agnostique (web components) | **Maintenance mode** depuis 2024, pas de roadmap active               |
| **MUI (`@mui/material`)**                                                    | Lib React, la plus populaire                 | React                       | **Très actif**, mais ancrée Material **2**, pas de M3 livré           |
| **Angular Material**                                                         | Lib officielle Angular (Material Components) | Angular                     | **Actif** (maintenu par l'équipe Angular), aligné M3                  |
| **Tokens M3 custom + web components/CSS** (incl. `material-color-utilities`) | Approche « do-it-yourself »                  | Agnostique                  | Les _briques_ (MCU) sont **actives**, l'assemblage est à votre charge |
...
| Repo local           | Package                              | Version constatée                              |
| -------------------- | ------------------------------------ | ---------------------------------------------- |
| `material-web/`      | `@material/web`                      | **2.4.1** (dépend de `lit ^2.8 \|\| ^3`)       |
| `material-ui/`       | `@mui/material`                      | **9.0.1** (monorepo `@mui/monorepo` 9.0.1)     |
| `material-tailwind/` | `@material-tailwind/react` / `-html` | **2.1.10** / **2.3.2** (v3 en beta hors arbre) |
| `shadcn-ui/`         | `ui` (registry/docs)                 | repo de référence shadcn                       |
...
Colonnes : **MWC** = `@material/web` (Lit) · **MUI** = `@mui/material` (React) · **MT** = `material-tailwind` · **shadcn+TW** = shadcn-style + Tailwind · **DIY** = tokens M3 custom + web components/CSS (avec MCU).

| Critère                                      | MWC (`@material/web`)                                                                                         | MUI (React)                                 | material-tailwind             | shadcn + Tailwind                           | DIY (tokens M3 + WC/CSS)            |
| -------------------------------------------- | ------------------------------------------------------------------------------------------------------------- | ------------------------------------------- | ----------------------------- | ------------------------------------------- | ----------------------------------- |
| **Runtime / framework**                      | Web components (Lit), agnostique                                                                              | React only                                  | React + HTML                  | React (registry), Tailwind                  | Agnostique                          |
...
| ↳ tokens couleur/typo/shape/elevation/motion | Tokens M3 natifs (CSS custom props)                                                                           | Thème MUI ≠ tokens M3                       | Quelques tokens partiels      | À définir vous-même                         | À définir (basé spec `01-…`)        |
| ↳ HCT                                        | Oui (via MCU en amont)                                                                                        | Non natif                                   | Non                           | Non (ajout MCU possible)                    | **Oui** (MCU)                       |
| **Dynamic color**                            | Oui (theming via tokens générés MCU)                                                                          | Non (palette statique du thème)             | Non                           | Non — ajoutable via MCU                     | **Oui** (MCU au build/runtime)      |
| **Couverture composants**                    | **Partielle** : manquent data table, date/time picker, nav bar/rail/drawer, snackbar, tooltip, badge, search… | **Très large** (lib la plus complète React) | Large (300+ blocks en v3 PRO) | Large via primitives, mais à styliser       | Nulle au départ (vous construisez)  |
| **Theming**                                  | Tokens CSS, Theme Builder                                                                                     | API `createTheme` puissante (mais M2)       | Config Tailwind               | Tokens CSS + Tailwind, total contrôle       | Total contrôle                      |
| **Accessibilité**                            | Bonne (intégrée aux WC)                                                                                       | Bonne (mûre)                                | Variable                      | Bonne (primitives Radix/Base sous-jacentes) | À votre charge                      |
| **Maintenance 2026**                         | **Maintenance mode**, pas de roadmap                                                                          | **Très active**                             | Beta lente                    | **Très active** (modèle, pas dépendance)    | Dépend de MCU (actif) + vous        |
...
### Scénario 1 — `@material/web` (Lit) + tokens M3 + Material Theme Builder
...
### Scénario 3 — Registry shadcn-style + Tailwind + tokens M3 custom + MCU (dynamic color)
...
- MWC maintenance mode — https://github.com/material-components/material-web/discussions/5642
- MWC repo & README — https://github.com/material-components/material-web
...
- MUI direction 2026 (Base UI, pauses) — https://mui.com/blog/2026-and-beyond/
- material-color-utilities — https://github.com/material-foundation/material-color-utilities · https://www.npmjs.com/package/@material/material-color-utilities
...
- material-tailwind (v3 beta) — https://github.com/creativetimofficial/material-tailwind
