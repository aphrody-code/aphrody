...
- **Impedance mismatch:** Emotion `sx`/`theme` vs `--md-sys-*` Shadow-DOM tokens
  is a fundamental design-language conflict. The measured migration wall is
  already `sx`/Emotion styling — forking
...
| **Migration value for consumers** | **This is the actual product.** Consumers get _off_ MUI: codemods (`transforms/orchestrator.ts` + `icons.ts`, 96% icon auto-map vs 4253-glyph table), `mui-m3-map.json`, the lint plugin, and anonymized measurements quantifying the real effort (the `sx` wall). |
...
2. **Keep investing where the product actually is — migration.** The measured
   wall is `sx`/Emotion, not component coverage. Prioritize:
