...
# `@material/web` — Composants Web matériels de Google
...
> « **MWC est en mode maintenance en attente de nouveaux responsables** » — `material-web/README.md`, ligne 21, et `material-web/docs/roadmap.md`.
...
> Saisie semi-automatique, badge, bannière, barre d'application inférieure, feuille inférieure, carte, tableau de données, sélecteur de date, barre de navigation, tiroir de navigation, rail de navigation, recherche, bouton segmenté, barre de collations, sélecteur de temps, barre d'application supérieure, info-bulle.
...
| Catégorie           | Balises                                                                                                                                                | Chemin repo                                                                       |
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------- |
| Boutons             | `md-elevated-button`, `md-filled-button`, `md-filled-tonal-button`, `md-outlined-button`, `md-text-button`                                             | `matériel-web/bouton/`                                                            |
| FAB                 | `md-fab`, `md-branded-fab`                                                                                                                             | `matériel-web/fab/`                                                               |
| Boutons d'icônes    | `bouton-icône-md`, `bouton-icône-rempli-md`, `bouton-icône-tonal-rempli-md`, `bouton-icône-contourné-md`                                               | `matériel-web/iconbutton/`                                                        |
| Sélection           | `md-checkbox`, `md-radio`, `md-switch`, `md-slider`                                                                                                    | `material-web/{case à cocher, radio, interrupteur, curseur}/`                     |
| Puces               | `md-chip-set`, `md-assist-chip`, `md-filter-chip`, `md-input-chip`, `md-suggestion-chip`                                                               | `matériel-web/chips/`                                                             |
...
| Listes & menus      | `md-list`, `md-list-item`, `md-menu`, `md-menu-item`, `md-sub-menu`, `md-menu-group`                                                                   | `matériel-web/{liste,menu}/`                                                      |
| Conteneurs / divers | `md-dialog`, `md-divider`, `md-elevation`, `md-focus-ring`, `md-ripple`, `md-icon`, `md-item`                                                          | `material-web/{dialogue, diviseur, élévation, mise au point, ondulation, icône}/` |
| Progrès             | `md-progrès-circulaire`, `md-progrès-linéaire`                                                                                                         | `matériel-web/progrès/`                                                           |
| Onglets             | `md-tabs`, `md-primary-tab`, `md-secondary-tab`                                                                                                        | `matériel-web/onglets/`                                                           |
...
| Balises                                                            | Chemin repo                                          |
| ------------------------------------------------------------------ | ---------------------------------------------------- |
| `md-badge`                                                         | `matériel-web/labs/badge/`                           |
| `md-elevated-card`, `md-filled-card`, `md-outline-card`, `md-card` | `matériel-web/labs/carte/`                           |
| `md-navigation-bar`, `md-navigation-tab`                           | `material-web/labs/navigationbar/`, `navigationtab/` |
| `md-navigation-tiroir`, `md-navigation-tiroir-modal`               | `material-web/labs/navigationdrawer/`                |
| `bouton-segmenté-décrit-md`, `jeu de boutons-segmentés-décrits-md` | `material-web/labs/segmentedbutton(set)/`            |
| `md-split-bouton`                                                  | `matériel-web/labs/splitbutton/`                     |
...
## 7. Fourchette locale (aphrody-code)
...
| Catégorie          | Balises                                                                                                                            | Chemin repo                                                 |
| ------------------ | ---------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------- |
| Communication      | `md-snackbar` (API Popover, couche supérieure), `md-loading-indicator` (M3 Expressive)                                             | `material-web/snackbar/`, `loadingindicator/`               |
| Navigation         | `md-navigation-rail` (+ `-item`), `md-top-app-bar` (CSS piloté par défilement), `md-bottom-app-bar`, `md-search-bar`, `md-toolbar` | `material-web/{navigationrail,appbar,search,toolbar}/`      |
| Conteneurs         | `md-bottom-sheet`, `md-side-sheet`, `md-carousel` (+ `-item`)                                                                      | `material-web/{feuille, carrousel}/`                        |
...
| Layout adaptatif   | `md-scaffold`, `md-pane`, `md-list-detail`, `md-supporting-pane` (ResizeObserver → `size-class`)                                   | `matériel-web/layout/`                                      |
| Typographie        | `md-type` (échelle de type M3 + axes animables Google Sans Flex, mode `code` → Google Sans Code)                                   | `matériel-web/typographie/`                                 |
| Effets             | `md-webgpu-canvas` (changement de spectre WGSL/étincelle/lueur, CSS de secours)                                                    | `matériel-web/effets/`                                      |
...
| Balises                                            | Chemin repo                   |
| -------------------------------------------------- | ----------------------------- |
| `md-info-bulle`                                    | `matériel-web/info-bulle/`    |
| `md-panneau d'extension`, `md-accordéon`           | `matériel-web/expansion/`     |
| `md-grid-list`, `md-grid-tile`                     | `matériel-web/gridlist/`      |
| `md-table` (tri de colonnes)                       | `matériel-web/table/`         |
| ``md-pager''                                       | `matériel-web/paginateur/`    |
| `md-stepper`, `md-étape`                           | `matériel-web/stepper/`       |
| `md-autocomplete`                                  | `matériel-web/autocomplete/`  |
| `md-tree`, `md-tree-item`                          | `matériel-web/arbre/`         |
| `md-virtual-scroller` (défilement virtuel Gap CDK) | `matériel-web/virtualscroll/` |
...
### 7.4 Plateforme Web moderne et jetons

- **`md-snackbar` → API Popover** (`popover="manual"`, couche supérieure, `@starting-style` + `transition-behavior:allow-discrete`), repli en ligne.
- **`md-top-app-bar` → CSS piloté par défilement** (`animation-timeline: scroll(...)`, piloté par le compositeur, gardé par `@supports`), JS de secours pour Firefox.
...
- **Agnostique du framework, basé sur des standards** : éléments personnalisés + Shadow DOM, encapsulation forte, framework pas de verrouillage.
...
- Dépôt local : `material-web/README.md`, `material-web/package.json`, `material-web/docs/roadmap.md`, `material-web/docs/intro.md`, `material-web/docs/theming/{color,shape,typography}.md`, `material-web/docs/components/`, `material-web/APHRODY-M3.md`, `material-web/all.ts`, `material-web/aphrody-{components,labs}.ts`.
- [Discussion n°5642 — MWC est en mode maintenance (GitHub)](https://github.com/material-components/material-web/discussions/5642)
...
- [Matériel 3 (m3.material.io)](https://m3.material.io/)
- [Matériau angulaire](https://material.angular.io/)
