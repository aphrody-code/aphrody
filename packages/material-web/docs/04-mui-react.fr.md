...
- Build orchestré par Lerna (`lerna run build`) ; cache Nx (`nx.json`) ; espaces de travail (`pnpm-workspace.yaml`).
- Description officielle (`packages/mui-material/package.json`) : _« Material UI est une bibliothèque de composants React open source qui implémente le Material Design de Google. »_
...
| ---------------------------------------------------------- | ------------------------------------------------------ | ------------ | -------------------------------------------------------------------------------------- |
| `@mui/matériel`                                            | `packages/mui-matériel`                                | 9.0.1        | Composants Material Design (le cœur grand public).                                     |
| `@mui/système`                                             | `packages/mui-système`                                 | 9.0.1        | Utilitaires CSS-in-JS de bas niveau : `sx`, `styled`, `Box`, `Stack`, accès au thème.  |
| `@mui/styled-engine`                                       | `packages/moteur-style-mui`                            | 9.0.0        | Wrapper `styled()` au-dessus de **Emotion** (moteur de style par défaut).              |
...
| `@mui/lab`                                                 | `paquets/mui-lab`                                      | 9.0.0-bêta.3 | Laboratoire de composants instables.                                                   |
...
| `pigment-css-react` / `pigment-réagir`                     | `packages/pigment-css-react`, `packages/pigment-react` | alpha        | Moteur de style **zero-runtime** (voir §6, **en pause**).                              |
| `@mui/codemod`, `@mui/envinfo`, `@mui/types`, `@mui/utils` | `paquets/mui-*`                                        | —            | Outillage : codemods de migration, diagnostics, types partagés, utilitaires.           |
...
| Aspect                  | Material Design 2 (ce que fait MUI)       | Conception matérielle 3 / Matériel vous                                                       |
| ----------------------- | ----------------------------------------- | --------------------------------------------------------------------------------------------- |
| Statut dans MUI v9      | ✅ implémenté                             | ❌ non implémenté, non planifié                                                               |
| Couleur                 | Palette fixe (primaire/secondaire…)       | Color roles + **HCT** + **dynamic color** (palette dérivée d'une couleur source)              |
| Jetons                  | `thème.palette`, `thème.typographie` (M2) | Concevoir des jetons M3 (`md.sys.color.*`, `md.sys.typescale.*`)                              |
...
- Packages locaux : `packages/pigment-css-react`, `packages/pigment-react`, et le pont `packages/mui-material-pigment-css` (même API `styled`/theming que Material UI).
...
| -------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| Entrées                    | Bouton, ButtonGroup, Case à cocher, Radio, Commutateur, Curseur, TextField, Sélection, Saisie semi-automatique, Évaluation |
| Affichage des données      | Avatar, Badge, Puce, Diviseur, Liste, Tableau, Info-bulle, Typographie, Icône                                              |
| Retour                     | Alerte, Toile de fond, CircularProgress, LinearProgress, Squelette, Snackbar, Boîte de dialogue                            |
| Surfaces                   | AppBar, Accordéon, Carte, Papier                                                                                           |
| Navigation                 | BottomNavigation, fil d'Ariane, tiroir, lien, menu, pagination, pas à pas, onglets, SpeedDial                              |
| Mise en page / utilitaires | Boîte, Conteneur, Grille, Pile, ImageList, CssBaseline, ClickAwayListener, Modal, Popover, Popper                          |
...
- Theming puissant : variables CSS, `color-mix()` (v9), modes clair/sombre, remplacements par composant.
...
- Conception matérielle 3 : <https://m3.material.io/>
