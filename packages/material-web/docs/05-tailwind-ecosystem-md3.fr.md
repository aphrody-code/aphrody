...
## 1. materials-tailwind : exploration du repo
...
| Emballer                     | Chemin                                                 | Version    | Rôle                                   |
| ---------------------------- | ------------------------------------------------------ | ---------- | -------------------------------------- |
| `@material-tailwind/react`   | `matériel-tailwind/packages/matériau-tailwind-react/`  | **2.1.10** | Composants React + ThemeProvider       |
| `@material-tailwind/html`    | `material-tailwind/packages/material-tailwind-html/`   | **2.3.2**  | Plugin Tailwind + CSS/JS pour HTML pur |
| `créer du matériel-tailwind` | `material-tailwind/packages/create-material-tailwind/` | —          | CLI d'échafaudage                      |
...
C'est une approche **Tailwind v3** (config JS, `tailwind.config.js`, directives `@tailwind base/components/utilities` dans `material-tailwind/packages/material-tailwind-html/styles/tailwind.css`). `tailwindcss` est dans devDependency `3.3.6` (racine) / `^3.2.4` (html).

### 1.3 Le système de thème (tokens)

Les tokens de base se trouvent dans `material-tailwind/packages/material-tailwind-react/src/theme/base/` :

- `colors.js` — palette de couleurs
- `typographie.js` — `fontFamily`
- `shadows.js` — `boxShadow`
- `breakpoints.js` — `écrans`
...
> "Projet démarré à partir de Tailwind CSS et **Material Design 2**"
> "Ajout du design de **Material Design 2** à l'aide de Tailwind CSS"
...
Dépôt `shadcn-ui/` = monorepo de `shadcn/ui` (CLI `packages/shadcn/`, application de doc/registry `apps/v4/`).
...
| Critère                                             | matériau-vent arrière                   | Tailwind CSS nu (v4)                     | shadcn/ui                                 |
| --------------------------------------------------- | --------------------------------------- | ---------------------------------------- | ----------------------------------------- |
| **Nature**                                          | Libération de composants (React + HTML) | Utilitaire moteur/CSS                    | Registre de composants (copier-coller)    |
| **Version (clonée)**                                | `réagir@2.1.10`, `html@2.3.2`           | `4.3.0`                                  | monorepo (apps/v4)                        |
| **Vent arrière de base**                            | v3 (`withMT()`, configuration JS)       | v4 ($`@theme`, CSS-first)                | v4 (`@thème en ligne`)                    |
| **Référence Material**                              | **MD2 "inspiré"**                       | Aucune (neutre)                          | Aucune (neutre)                           |
...
| **Couleur dynamique (graine → schéma)**             | Non                                     | Non                                      | Non                                       |
| **Élévation Material**                              | Partiel (ombres + ripple)               | Non                                      | Non (ombres neutres)                      |
| **A11y des primitives**                             | Maison (@floating-ui`)                  | N / A                                    | **Base / Base UI** (fort)                 |
| **Propriété du code / customisation**               | Fable (lib npm)                         | Total                                    | **Totale** (code copié)                   |
| **Entretien 2026**                                  | Faible / v3 gelée en beta               | Très active                              | Très active                               |
| **Conformité MD3 réelle**                           | **Faible** (c'est du MD2)               | **Nulle telle quelle**, mais socle idéal | **Nulle telle quelle**, mais modèle idéal |
| **Licence**                                         | AVEC                                    | AVEC                                     | AVEC                                      |
...
### 5.1 Mappage des tokens : `--md-sys-*` → `@theme`
...
| -------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| **HCT (Teinte-Chroma-Tone)**                                                           | M3 définit ses couleurs dans l'espace HCT (perception-uniforme Google), pas en OKLCH/HSL | Pré-calculer hors-build (lib `@material/material-color-utilities`) et **émettre du OKLCH/hex** dans les `--md-sys-color-*`               |
| **Couleur dynamique (graine → schéma complet)**                                        | M3 génère ~26 rôles light+dark depuis 1 couleur source via les palettes tonales          | Script de build qui prend un seed, appelle `material-color-utilities`, écrit le CSS des tokens. Tailwind ne fait pas ce calcul           |
| **Palettes de tons 0 à 100**                                                           | Base de la dérivation M3 (containers, surfaces)                                          | Générées par le même script ; non fournies par Tailwind (qui a des échelles 50–950 non-tonales)                                          |
| **Couches d'état** (survol/mise au point/appui = superposition en couleur à 8/12/16 %) | Modèle d'interaction M3 standardisé                                                      | Utilitaires/plugin personnalisé : superposition de pseudo-éléments `color-mix(in oklch, var(--md-sys-color-on-surface) 8%, transparent)` |
| **Élévation tonale + ombre**                                                           | M3 combine ombre _et_ teinte de surface selon le niveau                                  | Jetons `--md-sys-elevation-*` + variation de `surface-container-*`                                                                       |
| **Typo scale M3** (affichage des rôles/titre/titre/corps/étiquette × 3)                | Échelle nommée, pas une simple `text-sm/lg`                                              | Mapper et `--text-*` personnalisé dans `@theme`                                                                                          |
| **Ondulation / mouvement M3**                                                          | Animation Material (easing/durée standardisés)                                           | JS (cf. `material-ripple-effects`) + tokens d'easing/durée                                                                               |
...
3. **Exposition** : `@theme inline` mappe `--md-sys-*` → utilitaires (`--color-primary`, `--radius-md3-*`, `--shadow-md3-*`).
...
5. **Couches d'état + ondulation + mouvement** : plugin/utilitaires personnalisés + `color-mix()`.
...
- `material-tailwind/packages/material-tailwind-react/package.json` · `.../src/components/` (34 composants) · `.../src/theme/base/{couleurs, typographie, ombres, points d'arrêt}.js` · `.../src/theme/components/button/{buttonFilled,buttonGradient,buttonOutlined,buttonText,index}.ts`
...
- [Communiqués — creativetimofficial/material-tailwind](https://github.com/creativetimofficial/material-tailwind/releases)
- [Matériel Tailwind v3](https://www.material-tailwind.com/v3)
- [Installer le matériel Tailwind — Discussion tailwindlabs/tailwindcss n° 15958 (conflit Tailwind v4)](https://github.com/tailwindlabs/tailwindcss/discussions/15958)
