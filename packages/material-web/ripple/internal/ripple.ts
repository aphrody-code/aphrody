...
export class Ripple extends LitElement implements Attachable {
  ...
  private determineRippleSize(rect: DOMRect) {
  ...
  private getTranslationCoordinates(rect: DOMRect, positionEvent?: Event) {
    const { height, width } = rect;
    // `?? 1` may be removed once `currentCSSZoom` is widely available.
    const zoom = this.currentCSSZoom ?? 1;
    // end in the center
    ...
    if (positionEvent instanceof PointerEvent) {
      startPoint = this.getNormalizedPointerEventCoords(positionEvent, rect);
    } else {
      startPoint = {
        x: width / zoom / 2,
        y: height / zoom / 2,
  ...
  private startPressAnimation(positionEvent?: Event) {
    ...
    // Single getBoundingClientRect() shared across size and position math
    // to avoid multiple layout-flush reads per press.
    const rect = this.getBoundingClientRect();
    this.determineRippleSize(rect);
    const { startPoint, endPoint } = this.getTranslationCoordinates(rect, positionEvent);
