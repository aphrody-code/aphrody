 ...
 */
/**
 * Module-level cache for scrollbar dimensions. Populated lazily on the first
 * menu open and invalidated on `resize` events. This avoids a DOM
 * insert/measure/remove cycle on every `position()` call.
 */
let cachedScrollbarDims: { block: number; inline: number } | null = null;

function getScrollbarDims(): { block: number; inline: number } {
  if (cachedScrollbarDims !== null) {
    return cachedScrollbarDims;
  }
  const div = document.createElement("div");
  div.style.cssText = "opacity:0;position:fixed;display:block;inset:0";
  document.body.appendChild(div);
  const rect = div.getBoundingClientRect();
  div.remove();
  cachedScrollbarDims = {
    block: window.innerHeight - rect.bottom,
    inline: window.innerWidth - rect.right,
  };
  return cachedScrollbarDims;
}
...
export class SurfacePositionController implements ReactiveController {
  ...
  async position() {
    ...
    // Use the module-level cached scrollbar dimensions (invalidated on resize)
    // instead of inserting/measuring/removing a DOM element on every open.
    const { block: blockScrollbarHeight, inline: inlineScrollbarWidth } = getScrollbarDims();
