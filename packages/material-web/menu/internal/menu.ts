...
export abstract class Menu extends LitElement {
   ...
   */
  @property() positioning: "absolute" | "fixed" | "document" | "popover" = "absolute";
  /**
  ...
  private readonly menuId = `md-menu-${Math.floor(Math.random() * 1e9)}`;
   ...
   */
  private readonly menuPositionController = new SurfacePositionController(this, () => {
    return {
  ...
  override updated(changed: PropertyValues<Menu>) {
    ...
    if (supportsAnchor) {
      this.style.setProperty("--md-menu-x-offset", `${this.xOffset}px`);
      this.style.setProperty("--md-menu-y-offset", `${this.yOffset}px`);

      if (changed.has("open") || changed.has("anchorElement")) {
        const oldAnchor = changed.get("anchorElement") as
          | (HTMLElement & Partial<SurfacePositionTarget>)
          | null;
        if (oldAnchor && oldAnchor !== this.anchorElement) {
          oldAnchor.style.removeProperty("anchor-name");
        }

        if (this.open && this.anchorElement) {
          this.anchorElement.style.setProperty("anchor-name", `--${this.menuId}`);
          this.style.setProperty("--md-menu-anchor-name", `--${this.menuId}`);
        } else if (!this.open && this.anchorElement) {
          this.anchorElement.style.removeProperty("anchor-name");
          this.style.removeProperty("--md-menu-anchor-name");
        }
      }
    }
  }
  ...
  private readonly handleFocusout = async (event: FocusEvent) => {
    ...
    if (event.relatedTarget) {
      ...
      if (
        ...
        (this.pointerPath.length !== 0 && isElementInSubtree(event.relatedTarget, anchorEl))
      ) {
        return;
      }
  ...
  private async animateOpen() {
    ...
    const upPositionCorrectionAnimation = slotEl.animate(
      [{ transform: openingUpwards ? `translateY(-${height}px)` : "" }, { transform: "" }],
  ...
  private animateClose() {
    ...
    const DELAY_BETWEEN_ITEMS =
      (FULL_DURATION - ITEM_OPACITY_INITIAL_DELAY - ITEM_OPACITY_DURATION) / children.length;

    // The mock has the animation shrink to 35%
    const surfaceHeightAnimation = surfaceEl.animate(
      [{ height: `${height}px` }, { height: `${height * END_HEIGHT_PERCENTAGE}px` }],
      {
        duration: FULL_DURATION,
    ...
    const surfaceOpacityAnimation = surfaceEl.animate([{ opacity: 1 }, { opacity: 0 }], {
  ...
  close() {
    this.open = false;
    const maybeSubmenu = this.slotItems as Array<HTMLElement & { close?: () => void }>;
