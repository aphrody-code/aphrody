...
export class Field extends LitElement {
      ...
      <div class="field ${classMap(classes)}">
        <div class="container-overflow">
          ...
          <div class="container">
            ...
            <div class="middle">
              <div class="label-wrapper">${restingLabel} ${outline ? nothing : floatingLabel}</div>
              <div class="content">
                <slot></slot>
              </div>
            </div>
  ...
  protected override updated(changed: PropertyValues<Field>) {
    ...
    if (this.refreshErrorAlert || this.disableTransitions) {
      // Both flags need to be cleared after an animation frame. Consolidate
      // into a single rAF to avoid scheduling two microtask/paint callbacks
      // when both are true simultaneously (e.g. disabled-while-errored).
  ...
  private renderLabel(isFloating: boolean) {
    ...
    return html`
      <span class="label ${classMap(classes)}" aria-hidden=${!visible}>${labelText}</span>
    `;
  }
