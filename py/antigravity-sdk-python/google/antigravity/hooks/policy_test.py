...
class ShortCircuitTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_first_match_wins_within_deny_group(self):
        ...
        hook = policy.enforce(
            [
                policy.deny(
                    "run_command", when=counting_predicate, name="first"
                ),
                policy.deny(
                    "run_command", when=counting_predicate, name="second"
                ),
            ]
        )
        ctx = hooks.HookContext()
    ...
    async def test_skips_non_matching_predicate(self):
        ...
        hook = policy.enforce(
            [
                policy.deny(
                    "run_command", when=lambda args: False, name="skip-me"
                ),
                policy.deny(
                    "run_command", when=lambda args: True, name="catch-me"
                ),
            ]
        )
        ctx = hooks.HookContext()
...
class PredicateTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_sync_predicate_true(self):
        ...
        hook = policy.enforce(
            [
                policy.deny(
                    "run_command",
                    when=lambda args: args.get("CommandLine", "").startswith(
                        "rm"
                    ),
                ),
            ]
    ...
    async def test_predicate_exception_matches_fail_closed(self):
        ...
        hook = policy.enforce(
            [
                policy.deny(
                    "run_command", when=exploding_predicate, name="broken"
                ),
            ]
        )
        ctx = hooks.HookContext()
    ...
    async def test_parameterless_predicate(self):
        ...
        hook = policy.enforce(
            [
                policy.deny(
                    "run_command", when=no_args_predicate, name="no-args"
                ),
            ]
        )
        ctx = hooks.HookContext()
    ...
    async def test_parameterless_predicate_allow(self):
        ...
        hook = policy.enforce(
            [
                policy.allow(
                    "run_command", when=my_predicate, name="paramless-allow"
                ),
                policy.deny("*"),
            ]
        )
...
class IntegrationWithHookRunnerTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_policy_hook_in_hook_runner(self):
        ...
        from google.antigravity.hooks import (
            hook_runner,  # pylint: disable=g-import-not-at-top
        )
...
class WorkspaceOnlyTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_prevents_path_prefix_attack(self):
        ...
        result = await hook.run(
            ctx,
            _make_tool_call("view_file", path="/tmp/workspace-evil/file.txt"),
        )
        self.assertFalse(result.allow)
...
class PolicyPathScopingDirectTest(absltest.TestCase):
    ...
    def setUp(self):
        ...
        self.temp_dir_path = pathlib.Path(
    ...
    def test_secure_normalize_path_resolves_existing_symlinks(self):
        ...
        resolved_path = policy._secure_normalize_path(
            str(symlink_dir / "file.txt")
        )
...
class McpPolicyTest(unittest.IsolatedAsyncioTestCase):
    ...
    def test_ask_user_mcp_builder_specific_tools(self):
        ...
        policies = policy.ask_user(
            self.mcp_config, ["calc"], handler=dummy_handler
        )
        self.assertIsInstance(policies, list)
        (p,) = policies  # Implicitly asserts length is exactly 1
        self.assertEqual(p.tool, "math/calc")
    ...
    def test_mcp_tools_string_type_guard(self):
        ...
        self.assertIn(
            "mcp_tools must be a sequence of strings", str(ctx.exception)
        )
