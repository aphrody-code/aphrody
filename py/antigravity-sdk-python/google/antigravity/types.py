...
class ChatResponse:
    ...
    def chunks(self) -> AsyncIterator[StreamChunk | ToolCall | ToolResult]:
        ...
        async def _chunks_gen() -> AsyncIterator[
    ...
    async def text(self) -> str:
        ...
        return "".join(
            chunk.text for chunk in chunks if isinstance(chunk, Text)
        )
...
def _read_file_safely(path: str | pathlib.Path) -> bytes:
    ...
    try:
        ...
        raise OSError(
            f"Failed to read file at path '{file_path}': {exc}"
        ) from exc
...
def from_file(
    ...
    return media_cls(
        data=data, mime_type=mime_guess, description=description
