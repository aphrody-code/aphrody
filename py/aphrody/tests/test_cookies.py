...
def test_cookie_repr_does_not_leak() -> None:
    c = Cookie("cookie_name", "secret_cookie_value_12345", ".google.com")
    rep = repr(c)
    assert "secret_cookie_value_12345" not in rep
    assert "secret_c..." in rep
    assert "cookie_name" in rep

    jar = CookieJar([c])
    jar_rep = repr(jar)
    assert "secret_cookie_value_12345" not in jar_rep
    assert "secret_c..." in jar_rep
    assert "cookie_name" in jar_rep
