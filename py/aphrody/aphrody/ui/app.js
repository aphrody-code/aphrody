...
const _gemIcon = document.getElementById("gemIcon");
...
// Theme Management (Supports fluid dark-mode transitions and persistence)
function initTheme() {
  ...
  if (savedTheme === "dark") {
    ...
    // If no preference is saved, respect system preference and don't add class
...
clearLogBtn.addEventListener("click", () => {
  chatLog.innerHTML = `<div style="text-align: center; color: var(--md-sys-color-on-surface-variant); font-size: 13px; margin: auto;">Journal effacé. Parlez à l'assistant pour commencer.</div>`;
});
...
function updateAppStatus(status) {
  ...
  if (status === "listening") {
    statusTextDisplay.textContent = "Écoute en cours...";
    statusTextDisplay.style.color = "var(--gemini-brand-blue)";
  } else if (status === "thinking") {
    statusTextDisplay.textContent = "Réflexion...";
    statusTextDisplay.style.color = "var(--gemini-brand-purple)";
  } else if (status === "speaking") {
    statusTextDisplay.textContent = "L'agent parle...";
    statusTextDisplay.style.color = "var(--gemini-brand-pink)";
  } else {
    statusTextDisplay.textContent = "Voix active - Parlez maintenant";
...
async function initAudio() {
  try {
    ...
    alert("L'accès au microphone est requis pour l'interface voix-à-voix : " + err.message);
    disconnect();
  }
}
...
function connect() {
  ...
  connBadgeText.textContent = "Connexion...";
  ...
  ws.onopen = async () => {
    ...
    connBadgeText.textContent = "Connecté";
  ...
  ws.onmessage = (event) => {
    ...
    try {
      ...
      if (msg.type === "status") {
        ...
        appendChatMessage("agent", `[Erreur : ${msg.message}]`);
      }
    } catch (e) {
  ...
  ws.onerror = (err) => {
    ...
    connBadgeText.textContent = "Erreur";
    disconnect();
  };
}
...
function disconnect() {
  ...
  // Reset badge
  connBadge.className = "connection-badge";
  connBadgeText.textContent = "Déconnecté";
  ...
  statusTextDisplay.textContent = "Appuyez sur la sphère pour vous connecter";
  statusTextDisplay.style.color = "var(--md-sys-color-on-surface-variant)";
}
