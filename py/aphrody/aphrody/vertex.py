...
class GeminiVertex:
    ...
    def generate_think(
        ...
        *,
        ...
        """Generate text with a thinking budget and return (thought, response).
        ...
        Returns:
            A tuple of (thought_process, final_response).
        """
        from google.genai import types as gx

        target_model = model or "gemini-2.0-flash"
        config = gx.GenerateContentConfig(
            thinking_config=gx.ThinkingConfig(thinking_budget=budget)
        )
            ...
            for candidate in response.candidates:
                if candidate.content and candidate.content.parts:
                    for part in candidate.content.parts:
                        # Extract thought process if explicitly marked by SDK
        ...
        # Fallback to tag parsing if the SDK has embedded them in the text
        if not thought_process and final_response:
            import re
        ...
        if not thought_process and not final_response:
            final_response = response.text or ""

        return thought_process, final_response
