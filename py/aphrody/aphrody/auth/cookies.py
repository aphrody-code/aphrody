...
class Cookie:
    ...
    def __repr__(self) -> str:
        """A safe representation that does not leak sensitive cookie values."""
        masked_val = f"{self.value[:8]}..." if self.value else ""
        return (
            f"Cookie(name={self.name!r}, value={masked_val!r}, "
            f"domain={self.domain!r}, path={self.path!r}, "
            f"secure={self.secure!r}, http_only={self.http_only!r}, "
...
class CookieJar:
    ...
    def __repr__(self) -> str:
        """A safe representation of the cookie jar."""
        return f"CookieJar(cookies={self.cookies!r})"
...
def save(jar: CookieJar, path: Path | None = None) -> Path:
    ...
    try:
        enforce_private_permissions(p.parent)
    except Exception:
        pass
    ...
    content = json.dumps(jar.to_list(), indent=2, ensure_ascii=False).encode(
        "utf-8"
    )
    try:
