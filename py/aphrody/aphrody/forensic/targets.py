...
def _expand(raw: str) -> Path:
    ...
    if os.name != "nt":
        ...
        # Convert %VAR% to $VAR and backslashes to forward slashes
        raw = re.sub(r"%([^%]+)%", r"$\1", raw)
        raw = raw.replace("\\", "/")

        # Define cross-platform variable fallbacks if not defined in environ
        env = dict(os.environ)
        home = str(Path.home())
        if "LOCALAPPDATA" not in env:
            env["LOCALAPPDATA"] = env.get("XDG_DATA_HOME") or str(
                Path(home) / ".local" / "share"
            )
        if "APPDATA" not in env:
            env["APPDATA"] = env.get("XDG_CONFIG_HOME") or str(
                Path(home) / ".config"
            )

        # Manually substitute mock environment variables
        for k, v in env.items():
            raw = raw.replace(f"${k}", v).replace(f"${{{k}}}", v)
