...
"""RAPTOR (Recursive Abstractive Processing for Tree-Organized Retrieval) Tree Builder.

Provides recursive text summarization and semantic hierarchical indexing.
Supports both classic GMM-based clustering and Psi merge-tree structures.
"""
...
class _PsiTreeNode:
    ...
    parent: Optional["_PsiTreeNode"] = None
...
class _PsiUnionFind:
    ...
    def union(self, i: int, j: int) -> bool:
        ...
        if self._rank[root_i] < self._rank[root_j]:
            ...
            self._ordered_extend(
                self._parent_chains[root_i], chain[higher_rank_idx:]
            )
            ...
            insert_point = chain[higher_rank_idx]
            self._ordered_extend(
                self._parent_chains[root_j], chain[higher_rank_idx:]
            )
            ...
            if not self._parent_chains[root_i]:
                self._parent_chains[root_i].append(root_i)
            self._ordered_extend(
                self._parent_chains[root_j], self._parent_chains[i][-1:]
            )
            self._rank[root_i] += 1
...
class RecursiveAbstractiveProcessing4TreeOrganizedRetrieval:
    ...
    def __init__(
        ...
        threshold: float = 0.1,
        max_errors: int = 3,
        tree_builder: str = RAPTOR_TREE_BUILDER,
        clustering_method: str = GMM_CLUSTERING_METHOD,
        ...
        if self._tree_builder not in SUPPORTED_TREE_BUILDERS:
            raise ValueError(
                f"Unsupported RAPTOR tree builder: {self._tree_builder}"
            )
        ...
        if self._clustering_method not in SUPPORTED_CLUSTERING_METHODS:
            raise ValueError(
                f"Unsupported RAPTOR clustering method: {self._clustering_method}"
            )
        self._psi_exact_max_leaves = max(2, int(psi_exact_max_leaves or 4096))
        self._psi_bucket_size = min(
            max(2, int(psi_bucket_size or 1024)), self._psi_exact_max_leaves
        )
    ...
    async def _chat(
        ...
        """Call the configured LLM with short retries."""
        last_exc = None
        for attempt in range(3):
            try:
                ...
                last_exc = exc
                logger.warning(
                    "RAPTOR LLM call failed on attempt %d/3: %s",
                    attempt + 1,
                    exc,
                )
                if attempt < 2:
        ...
        raise (
            last_exc
            if last_exc
            else Exception("LLM chat failed without exception")
        )

    async def _embedding_encode(self, txt: str) -> np.ndarray:
        """Encode text with the configured embedding function."""
        if asyncio.iscoroutinefunction(self._embed_fn):
            embds = await self._embed_fn([txt])
        else:
            embds = self._embed_fn([txt])

        if len(embds) < 1 or len(embds[0]) < 1:
            raise Exception("Embedding error: empty embeddings returned")
        return np.asarray(embds[0], dtype=np.float64)
    ...
    def _get_clusters_ahc(self, embeddings: np.ndarray) -> np.ndarray:
        ...
        if n_clusters <= 1:
            logger.info(
                "RAPTOR AHC: _get_clusters_ahc selected one cluster for %d embeddings",
                n,
            )
            return np.zeros(n, dtype=int)

        logger.info(
            "RAPTOR AHC: _get_clusters_ahc selected n_clusters=%d for %d embeddings",
            n_clusters,
            n,
        )
    ...
    async def _summarize_texts(self, texts: list[str], callback=None):
        """Summarize a cluster and return text plus embedding when successful."""
        cluster_content = "\n".join(texts)
        try:
            cnt = await self._chat(
                "You are a helpful assistant responsible for summarizing texts.",
                [
                    {
                        "role": "user",
            ...
            if self._error_count >= self._max_errors:
                raise RuntimeError(
                    f"RAPTOR aborted after {self._error_count} errors. Last error: {exc}"
                ) from exc
            return None
    ...
    def _rank_leaf_pairs(self, leaves: list[_PsiTreeNode]) -> np.ndarray:
        ...
        node_embeddings = np.asarray(
            [leaf.embedding for leaf in leaves], dtype=np.float64
        )
    ...
    def _split_psi_buckets(
        self, nodes: list[_PsiTreeNode]
    ) -> list[list[_PsiTreeNode]]:
        """Split large Psi inputs so exact pair ranking is bounded per bucket."""
        if len(nodes) <= self._psi_bucket_size:
            return [nodes]

        node_embeddings = self._normalize_embeddings(
            np.asarray([node.embedding for node in nodes], dtype=np.float64)
        )
        ...
        while groups:
            ...
            fanout = min(
                max(2, int(np.ceil(len(group) / self._psi_bucket_size))),
                len(group),
                32,
            )
            group_embeddings = node_embeddings[group]
            center_idx = np.linspace(
                0, len(group_embeddings) - 1, num=fanout, dtype=int
            )
    ...
    def _assign_prototype_embeddings(self, node: _PsiTreeNode) -> np.ndarray:
        ...
        embeddings = np.asarray(
            [
                self._assign_prototype_embeddings(child)
    ...
    def _create_psi_parent(
        self, index: int, children: list[_PsiTreeNode]
    ) -> _PsiTreeNode:
    ...
    def _rebalance_psi_tree(
        self, root: _PsiTreeNode, next_index: int
    ) -> tuple[_PsiTreeNode, int]:
                ...
                for start in range(0, len(node.children), max_children):
                    ...
                    if len(batch) == 1:
                        ...
                        grouped_children.append(
                            self._create_psi_parent(next_index, batch)
                        )
    ...
    def _build_exact_psi_structure(
        ...
        for parent_idx, child_indices in children_by_parent.items():
            ...
            parent.children = [
                local_nodes[child_idx] for child_idx in child_indices
            ]
        ...
        roots = [
            ...
            if parent_idx == -1 and idx in local_nodes
        ]
    ...
    def _build_bucketed_psi_structure(
        ...
        for bucket in buckets:
            bucket_root, next_index, bucket_merges = (
                self._build_psi_structure_from_nodes(bucket, next_index)
            )
        ...
        root, next_index, root_merges = self._build_psi_structure_from_nodes(
            bucket_roots, next_index
        )
    ...
    def _build_psi_structure_from_nodes(
        ...
        if len(nodes) <= self._psi_exact_max_leaves:
            return self._build_exact_psi_structure(nodes, next_index)
        return self._build_bucketed_psi_structure(nodes, next_index)

    def _build_psi_structure(
        self, chunks: list[tuple[str, list[float]]]
    ) -> tuple[_PsiTreeNode, list[_PsiTreeNode]]:
        ...
        root, next_index, merges = self._build_psi_structure_from_nodes(
            leaves, len(leaves)
        )
    ...
    async def _build_psi_layers(
        self, chunks: list[tuple[str, list[float]]], callback=None
    ):
        """Materialize Psi tree layers as summary chunks."""
        layers = [(0, len(chunks))]
        root, _ = self._build_psi_structure(chunks)

        for layer_idx, (_, nodes) in enumerate(
            sorted(self._psi_layers(root).items()), start=1
        ):
            ...
            async def summarize_node(node: _PsiTreeNode):
                ...
                if not texts:
                    logger.warning(
                        "RAPTOR Psi node %s skipped because it has no child text to summarize",
                        node.index,
                    )
                    return None
                ...
                if result is None:
                    logger.warning(
                        "RAPTOR Psi node %s skipped because summarization failed",
                        node.index,
                    )
                    return None
            ...
            tasks = [
                asyncio.create_task(summarize_node(node)) for node in nodes
            ]
            try:
                summarized_nodes = await asyncio.gather(
                    *tasks, return_exceptions=False
                )
            except Exception as e:
                logger.error("Error in RAPTOR Psi tree processing: %s", e)
                for task in tasks:
                    task.cancel()
            ...
            summarized_nodes = [
                node for node in summarized_nodes if node is not None
            ]
            for node in summarized_nodes:
                chunks.append((node.text, node.embedding.tolist()))
            ...
            if len(chunks) > layer_start:
                ...
                if callback:
                    callback(
                        msg=f"Build one Psi-RAG layer: {len(nodes)} -> {len(chunks) - layer_start}"
                    )
            else:
                logger.warning(
                    "RAPTOR Psi layer %d produced no summaries; stopping materialization",
                    layer_idx,
                )
                break
    ...
    async def __call__(
        ...
        if len(chunks) <= 1:
            return [], []
        chunks = [
            (s, a) for s, a in chunks if s and a is not None and len(a) > 0
        ]
        if len(chunks) <= 1:
            return chunks, [(0, len(chunks))]
        if self._tree_builder == PSI_TREE_BUILDER:
            logger.info(
                "RAPTOR: using %s tree builder for %d chunks",
                self._tree_builder,
                len(chunks),
            )
            return await self._build_psi_layers(chunks, callback)
        ...
        async def summarize(ck_idx: list[int]):
            ...
            if result is not None:
                text, embd = result
                chunks.append((text, embd.tolist()))
        ...
        while end - start > 1:
            ...
            if len(embeddings) == 2:
                ...
                if produced == 0:
                    logger.warning(
                        "RAPTOR layer produced no summaries; stopping materialization"
                    )
                    break
                if callback:
                    callback(
                        msg=f"Cluster one layer: {end - start} -> {produced}"
                    )
            ...
            if self._clustering_method == AHC_CLUSTERING_METHOD:
                logger.info(
                    "RAPTOR: using clustering_method=%s before _get_clusters_ahc",
                    self._clustering_method,
                )
                raw_labels = self._get_clusters_ahc(reduced_embeddings)
                raw_cluster_count = np.unique(raw_labels).size
                logger.info(
                    "RAPTOR AHC: _get_clusters_ahc produced n_clusters=%d",
                    raw_cluster_count,
                )
                if raw_cluster_count > 1:
                    adjusted = self._adjust_tree_nodes(
                        reduced_embeddings, raw_labels
                    )
                    adjusted_cluster_count = np.unique(adjusted).size
                    logger.info(
                        "RAPTOR AHC: _adjust_tree_nodes adjusted n_clusters=%d",
                        adjusted_cluster_count,
                    )
                else:
                    adjusted = raw_labels
                    logger.warning(
                        "RAPTOR AHC: _adjust_tree_nodes skipped because _get_clusters_ahc returned one cluster"
                    )
                ...
                n_clusters = self._get_optimal_clusters(
                    reduced_embeddings, random_state
                )
                if n_clusters == 1:
                    ...
                    lbls = [
                        np.where(prob > self._threshold)[0] for prob in probs
                    ]
                    lbls = [
            ...
            try:
                ...
                logger.error("Error in RAPTOR cluster processing: %s", e)
                for t in tasks:
                    t.cancel()
                await asyncio.gather(*tasks, return_exceptions=True)
                raise
            ...
            layers.append((end, len(chunks)))
            if callback:
                callback(msg=f"Cluster one layer: {end - start} -> {produced}")
            start = end
            end = len(chunks)
