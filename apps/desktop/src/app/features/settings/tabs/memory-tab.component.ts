...
@Component({
  ...
  template: `
    <section class="card">
      ...
      <p class="hint">
        L'agent conserve sa mémoire à long terme dans une base SQLite locale. Elle est lue à chaque
        conversation et alimente le contexte.
      </p>
      @if (stat(); as s) {
        <div class="grid">
          <div class="kv"><span>Fichier</span><b class="mono">{{ s.path }}</b></div>
          <div class="kv">
            <span>État</span>
            @if (s.exists) {
          ...
          @if (s.exists) {
            <div class="kv"><span>Taille</span><b>{{ size(s) }}</b></div>
            @if (s.modified_secs > 0) {
              <div class="kv"><span>Modifié</span><b>{{ modified(s) }}</b></div>
            }
          }
        </div>
      } @else if (statError()) {
    ...
    <section class="card">
      ...
      <p class="hint">
        Le trait <code>MemoryProvider</code> permet de copier des enregistrements entre fournisseurs
        (<code>aphrody memory migrate</code>). Les fournisseurs HTTP lisent leurs identifiants dans
        l'environnement.
      </p>
      <ul class="providers">
        <li><b>SqliteLocal</b><span>Base locale hors-ligne (par défaut, ci-dessus).</span></li>
        <li><b>Mem0</b><span>Service HTTP — clé <code>MEM0_API_KEY</code>.</span></li>
        <li><b>Honcho</b><span>Service HTTP (dialectic v3) — clé <code>HONCHO_API_KEY</code>.</span></li>
      </ul>
      @if (helpText()) {
