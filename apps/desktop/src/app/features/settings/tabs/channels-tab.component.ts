...
const GROUPS: Group[] = [
  {
    ...
    fields: [{ key: "DISCORD_BOT_TOKEN", label: "Jeton du bot Discord", secret: true, placeholder: "Bot token" }],
  },
  {
    title: "X (Twitter)",
  ...
  },
  {
    ...
    fields: [
      ...
      { key: "ELEVENLABS_API_KEY", label: "Clé ElevenLabs (TTS)", secret: true, placeholder: "Clé API" },
    ],
  },
  {
    title: "Slack",
  ...
  },
  {
    ...
    fields: [
      { key: "TELEGRAM_BOT_TOKEN", label: "Jeton du bot Telegram", secret: true, placeholder: "123456:ABC…" },
      { key: "TELEGRAM_CHAT_ID", label: "Chat ID par défaut", secret: false, placeholder: "-1001234567890" },
    ],
  },
  {
    title: "Matrix",
    ...
    fields: [
      { key: "MATRIX_HOMESERVER", label: "Homeserver", secret: false, placeholder: "https://matrix.org" },
      { key: "MATRIX_ACCESS_TOKEN", label: "Jeton d'accès", secret: true, placeholder: "Access token" },
      { key: "MATRIX_USER_ID", label: "User ID", secret: false, placeholder: "@moi:matrix.org" },
      { key: "MATRIX_ROOM_ID", label: "Room ID par défaut", secret: false, placeholder: "!abc:matrix.org" },
    ],
  },
];
  ...
  template: `
    <section class="card intro">
      ...
      <p class="hint">
        ...
        <code>~/.aphrody/channels.json</code>, hors du dépôt. Les secrets sont masqués : laissez un
        champ secret vide pour conserver la valeur existante.
      </p>
      @if (state(); as s) {
    ...
    @if (hostUnavailable()) {
      <section class="card"><p class="warn">La configuration des canaux nécessite l'application de bureau.</p></section>
    } @else if (loadError()) {
      <section class="card"><p class="warn">{{ loadError() }}</p></section>
    } @else if (state()) {
          ...
          @for (f of g.fields; track f.key) {
            <label class="field">
              ...
              <input
                ...
                [placeholder]="f.secret && isSet(f.key) ? '••• défini (laisser vide pour conserver)' : f.placeholder"
                [value]="value(f)"
