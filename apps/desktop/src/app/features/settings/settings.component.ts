                    ...
                    <section class="card">
                        ...
                        @if (account.account(); as a) {
                            @if (a.connected) {
                                ...
                                <p class="hint">
                                    Compte Google lié à aphrody (token agy / cookie Google). Les
                                    conversations utilisent ce compte via Gemini.
                                </p>
                            } @else {
                                <div class="account-row">
                                    ...
                                    <div class="who">
                                        <b>Non connecté</b>
                                        <span>{{
                                            account.error() || "Aucune session Google active."
                                        }}</span>
                                    </div>
                                </div>
                                <p class="hint">
                                    Lancez l'authentification Antigravity (agy) en arrière-plan pour
                                    régénérer la session, puis actualisez.
                                </p>
                            }
                        }
                        <button
                        ...
                        >
                            <mat-icon class="material-symbols-outlined">{{
                                account.loading() ? "progress_activity" : "refresh"
                            }}</mat-icon>
                    ...
                    <section class="card">
                        ...
                        <div class="options">
                            @for (o of backends; track o.id) {
                                <button
                                    class="option"
                                    [class.sel]="settings.backend() === o.id"
                                    (click)="settings.setBackend(o.id)"
                                >
                                    <mat-icon class="material-symbols-outlined">{{
                                        o.icon
                                    }}</mat-icon>
                                    <span class="opt-text"
                                        ><b>{{ o.label }}</b
                                        ><span>{{ o.desc }}</span></span
                                    >
                                    @if (settings.backend() === o.id) {
                                        <mat-icon class="material-symbols-outlined check"
                                            >check</mat-icon
                                        >
                                    }
                                </button>
                    ...
                    <section class="card">
                        ...
                        <div class="about-head">
                            ...
                            <div>
                                <b>aphrody</b>
                                <p class="tagline">
                                    Agent autonome — assistant IA propulsé par Gemini.
                                </p>
                            </div>
                        </div>
                        @if (meta(); as m) {
                            <div class="grid">
                                <div class="kv">
                                    <span>Version</span><b>{{ m.app_version }}</b>
                                </div>
                                <div class="kv">
                                    <span>Système</span><b>{{ m.target_os }}</b>
                                </div>
                                <div class="kv">
                                    <span>Architecture</span><b>{{ m.target_arch }}</b>
                                </div>
                                <div class="kv">
                                    <span>Famille</span><b>{{ m.family }}</b>
                                </div>
                            </div>
                        }
                        <p class="note">
                            Angular 21 + Angular Material 21 dans une coque Tauri, propulsé en local
                            par le binaire aphrody.
                        </p>
                    </section>
                </mat-tab>
            </mat-tab-group>
...
export class SettingsComponent implements OnInit {
    ...
    readonly backends: BackendOption[] = [
        ...
        { id: "web", label: "Gemini Web", desc: "Cookie Google signé, sans clé", icon: "language" },
        {
            id: "stub",
            label: "Hors-ligne",
            desc: "Réponse locale, aucun réseau",
            icon: "cloud_off",
        },
    ];
