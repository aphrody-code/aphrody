  ...
  template: `
      ...
      <div class="run-row">
        ...
        <div class="run-text">
          ...
          <span class="run-hint"
            ...
            sensibles (cookies, jetons, leveldb…) sont recensés sans jamais être ouverts.</span
          >
        </div>
      </div>
    ...
    @if (report(); as r) {
      <div class="tiles">
        @for (t of tiles(); track t.label) {
          <mat-card class="tile" appearance="outlined" [attr.data-tone]="t.tone">
            <mat-icon class="material-symbols-outlined tile-icon">{{ t.icon }}</mat-icon>
      ...
      <mat-card class="wrote-card" appearance="outlined">
        ...
        <code class="wrote-path mono" [matTooltip]="r.wrote" matTooltipClass="path-tip">{{ r.wrote }}</code>
        <p class="wrote-note">
    ...
    try {
      ...
      this.report.set({
        ...
        secret_meta_only_count:
          typeof parsed.secret_meta_only_count === "number" ? parsed.secret_meta_only_count : 0,
      });
    } catch (err) {
