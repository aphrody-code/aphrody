    ...
    <mat-card class="run-card" appearance="outlined">
      <div class="run-row">
        ...
        <div class="run-text">
          ...
          <span class="run-hint"
            >Énumération OSINT passive des sous-domaines via crt.sh + HackerTarget (aphrody dns).</span
          >
        </div>
      </div>
      <div class="run-input">
        <input
          ...
          spellcheck="false"
        />
        <button mat-raised-button color="primary" (click)="run()" [disabled]="running() || !domain().trim()">
        ...
        <div class="hdr-top">
          ...
          @if (r.hiddenCount > 0) {
            <span class="kv">
              ...
              <span class="v">{{ r.subdomains.length }} <span class="dim">/ {{ r.total }}</span></span>
            </span>
          }
        </div>
        @if (r.hiddenCount > 0) {
          <p class="note">
            ...
            Le CLI ne liste que les 10 premiers sous-domaines ; {{ r.hiddenCount }} autres ne sont pas détaillés.
          </p>
        }
      </mat-card>
          ...
          @if (filtered().length === 0) {
            ...
            <ul class="sub-grid">
              @for (sub of filtered(); track sub) {
                <li class="sub-item">
                  <mat-icon class="material-symbols-outlined sub-glyph">subdirectory_arrow_right</mat-icon>
  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      if (!parsed) {
        ...
        this.error.set(
          (res.stdout || res.stderr || "Sortie vide.").trim().slice(0, 2000),
        );
        return;
      }
