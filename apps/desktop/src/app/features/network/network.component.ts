...
@Component({
  ...
  template: `
    <div class="page">
      <header class="head">
        <div class="head-icon"><mat-icon class="material-symbols-outlined">dns</mat-icon></div>
        <div>
          <h1>Réseau</h1>
          <p>Reconnaissance DNS OSINT et recherche web native (<code>aphrody dns / search</code>).</p>
        </div>
      </header>
      ...
      <section class="raw-block">
        <button class="raw-toggle" (click)="rawOpen.set(!rawOpen())" [attr.aria-expanded]="rawOpen()">
          <mat-icon class="material-symbols-outlined">terminal</mat-icon>
          <span>Vue brute (sortie CLI)</span>
          <mat-icon class="material-symbols-outlined chevron">{{ rawOpen() ? "expand_less" : "expand_more" }}</mat-icon>
        </button>
        @if (rawOpen()) {
          <app-tool-runner
