        ...
        <mat-card class="run-card" appearance="outlined">
            <div class="run-row">
                ...
                <div class="run-text">
                    ...
                    <span class="run-hint"
                        >Format, sections + entropie, imports/exports, SHA-256 (aphrody re
                        triage).</span
                    >
                </div>
            </div>
        ...
        @if (report(); as r) {
                ...
                <div class="hdr-top">
                    ...
                    @if (r.arch) {
                        <span class="kv"
                            ...
                            ><span class="v">{{ r.arch }}</span></span
                        >
                    }
                    <span class="kv"
                        ><span class="k">Taille</span
                        ><span class="v">{{ humanSize(r.size) }}</span></span
                    >
                    @if (r.entry_point !== null) {
                        <span class="kv"
                            ><span class="k">Point d'entrée</span
                            ><span class="v mono">{{ hex(r.entry_point) }}</span></span
                        >
                    }
                </div>
                <div class="sha-row">
                    <span class="k">SHA-256</span>
                    <code class="sha mono" [matTooltip]="r.sha256" matTooltipClass="sha-tip">{{
                        shaShort()
                    }}</code>
                </div>
            </mat-card>
                ...
                <mat-card class="tbl-card" appearance="outlined">
                    ...
                    <table mat-table [dataSource]="r.sections" class="sec-table">
                        ...
                        <ng-container matColumnDef="vaddr">
                            ...
                            <td mat-cell *matCellDef="let s">
                                <code class="mono dim">{{ hex(s.vaddr) }}</code>
                            </td>
                        </ng-container>
                    ...
                    <p class="legend">
                        <span class="lg high"></span> ≥ {{ entropyHigh }} : probablement packé /
                        chiffré <span class="lg med"></span> ≥ {{ entropyMed }} : dense
                            ...
                            <mat-chip-set class="sym-chips" aria-label="Symboles importés">
                                @for (sym of r.imports; track $index) {
                                    <mat-chip disableRipple
                                        ><code class="mono">{{ sym }}</code></mat-chip
                                    >
                                }
                            </mat-chip-set>
                        </mat-card>
            ...
            @if (r.strings_sample.length > 0) {
                <mat-card class="str-card" appearance="outlined">
                    <button
                        class="block-head as-button"
                        (click)="stringsOpen.set(!stringsOpen())"
                        [attr.aria-expanded]="stringsOpen()"
                    >
