...
@Component({
  ...
  template: `
    <div class="card">
      <div class="card-head">
        ...
        <button class="refresh" (click)="load()" [disabled]="loading()" aria-label="Rafraîchir la version">
      ...
      @if (info(); as v) {
        ...
        <div class="kv">
          ...
          <div class="kv-row">
            ...
            <span class="kv-val">
              @if (isTauri) {
                <button class="link" (click)="openRepo(v.repo)">{{ v.repo }}</button>
              } @else {
                <a class="link" [href]="v.repo" target="_blank" rel="noopener noreferrer">{{ v.repo }}</a>
              }
            </span>
          </div>
        </div>
  ...
  styles: [
    `
      ...
      .chip.subtle {
        background: color-mix(in srgb, var(--mat-sys-on-primary-container) 18%, transparent);
        color: var(--mat-sys-on-primary-container);
        font-weight: 500;
      }
      .kv {
  ...
  async load(): Promise<void> {
    ...
    try {
      ...
      if (start < 0) {
        this.info.set(null);
        this.parseError.set(
          res.stderr.trim() || "La commande version n'a renvoyé aucun objet JSON exploitable.",
        );
        return;
      }
