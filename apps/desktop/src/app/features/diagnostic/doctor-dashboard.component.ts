...
@Component({
  ...
  template: `
    <div class="page">
      <header class="head">
        <div class="head-icon"><mat-icon class="material-symbols-outlined">monitor_heart</mat-icon></div>
        <div>
          <h1>Diagnostic</h1>
          <p>État du système, supply-chain et intégration A2A (<code>aphrody doctor</code>).</p>
        </div>
        <button class="refresh" (click)="load()" [disabled]="loading()" aria-label="Rafraîchir le diagnostic">
      ...
      @if (loading()) {
        ...
        <section class="verdict" [attr.data-h]="verdictHealth()">
          <mat-icon class="material-symbols-outlined verdict-glyph">{{ verdictIcon() }}</mat-icon>
        ...
        <div class="card">
          ...
          <div class="kv">
            <div class="kv-row">
              ...
              <span class="kv-val">
                <span class="inline-chip" [attr.data-h]="peerHealth()">{{ peerStatusLabel() }}</span>
              </span>
            </div>
            <div class="kv-row">
