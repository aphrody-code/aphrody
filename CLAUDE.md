...
> (client **grand public**, React + m3-react + TanStack, LLM custom shenron). Le **cœur Rust reste 100 % Rust**.
...
- **`pyo3` sur Windows sans Python réel** : `cargo clippy --workspace` (et tout build touchant `pyo3-build-config`) échoue avec `error: no Python 3.x interpreter found` — `python` sur le PATH est le stub Microsoft Store, pas un interpréteur. Le Python 3.14 installé par **uv** convient et correspond à `py/.python-version` : `PYO3_PYTHON="$(uv python find 3.14)" cargo clippy --workspace`. Sur cette machine : `C:/Users/aphro/AppData/Roaming/uv/python/cpython-3.14-windows-x86_64-none/python.exe`.
...
- **Les features opt-in ne sont vérifiées par personne par défaut.** `ocr`,
  `infer`, `magika`, `forensics`, `index`, `firefly` sont hors du set `default` :
  `cargo clippy --workspace --all-targets` passe donc au vert sur du code qui ne
  compile même pas sous `--features ocr`. Vécu le 2026-09-04 : un commit avait
  retiré deux fonctions de `ocr_cmd` en laissant leur appel, et rendu `run`
  synchrone sans retirer le `.await` — la feature était cassée depuis plusieurs
  jours sans qu'aucun gate ne bronche. Après tout changement touchant un module
  gated, lancer explicitement
  `cargo check -p aphrody --features "ocr,infer,index,forensics,firefly"`.
- **Rebuild du binaire = rebuild avec ses features.** Un `cargo build -p aphrody
  --bin aphrody` sans `--features ocr` produit un binaire où `aphrody ocr`
  n'existe plus — et clap ne dit pas « unknown subcommand » : l'argv non
  reconnue tombe dans le repli **A2A `auto_command`**, qui échoue sur
  `http://localhost:8788/jsonrpc: error sending request`. Le message ne parle
  jamais de features, donc on cherche un serveur A2A absent pendant que le vrai
  problème est le drapeau de build. Vérifier après tout rebuild : `aphrody ocr`
  doit lister `page/audit/clean/batch`.
...
- **Filtre de découverte plus strict que le filtre de correction** : mode d'échec qui ne lève aucune erreur et ne produit aucun symptôme — il fait juste disparaître des résultats. Vécu sur les databooks (2026-08-25) : un script d'analyse comptait `・` comme katakana bloquant là où le module de correction le tenait pour une frontière de mot ; les 54 occurrences de `プロリー` étant toutes bordées d'un `・`, le comptage rendait zéro et un `if (occ === 0) continue;` les écartait **en silence**. 25 planches sont restées fautives, invisibles dans tous les rapports. Règle : quand on écarte des candidats, **compter et journaliser ce qu'on écarte** ; un détecteur **annote, il ne filtre pas**. Corollaire mesuré : un crible appliqué **avant** la lecture des contextes (ici JMdict) masque ce qu'il rejette — `スラック` (Slug) écarté comme mot anglais alors que 55 de ses 64 occurrences sont des fiches de personnage. Détails : [`docs/databooks-hallucinations.md`](docs/databooks-hallucinations.md).
- **Corriger un corpus public** : une règle à 50 % de faux positifs est **pire que le défaut** — le défaut se voit, la fausse correction non. Toute famille corrigée doit venir avec son comptage mesuré, le contre-exemple cherché, et son test de non-régression. Détecteur : `shenron:apps/site/scripts/detecte-hallucinations.ts` (bloquant / signalé / **témoin** — une BAISSE d'un témoin est une régression : une règle a mangé du vrai texte).
