...
class LocalConnection(connection.Connection):
    ...
    async def send(self, prompt: types.Content | None) -> None:
        ...
        if prompt is None:
            ...
            event = localharness_pb2.InputEvent(
                complex_user_input=user_input_pb
            )
        ...
        try:
            async for raw_msg in self._ws:
                ...
                if event.HasField("step_update"):
                    ...
                    if (
                        step_obj.type == types.StepType.COMPACTION
                    ...
                    if (
                        ...
                        tc, op_ctx = self._pending_builtin_tool_calls.pop(
                            step_key
                        )
                        if self._hook_runner:
                            ...
                            self._run_in_background(
                                self._hook_runner.dispatch_post_tool_call(
                                    op_ctx, result
                                )
                            )
                    elif (
                        ...
                        _, op_ctx = self._pending_builtin_tool_calls.pop(
                            step_key
                        )
                        if self._hook_runner:
                            error = RuntimeError(
                            ...
                            self._run_in_background(
                                self._hook_runner.dispatch_on_tool_error(
                                    op_ctx, error
                                )
                            )
                    ...
                    if (
                        ...
                        if step_update.HasField("tool_confirmation_request"):
                            if tracker.mark_handled(
                                "tool_confirmation_request"
                            ):
                                self._run_in_background(
                                    self._handle_tool_confirmation_request(
                                        step_update
                                    )
                                )
                    ...
                    is_subagent = (
                        self._cascade_id
                        and tsu.trajectory_id != self._cascade_id
                    )
                    ...
                    if (
                        ...
                        if is_subagent:
                            ...
                            if self._hook_runner:
                                op_ctx = hooks.OperationContext(
                                    self._get_turn_context()
                                )
            ...
            if self._disconnecting:
                ...
                logging.info(
                    "WebSocket closed (code %s); normal shutdown.", e.code
                )
            else:
        ...
        try:
                ...
                if uq.HasField("multiple_choice"):
                    ...
                    questions_list.append(
                        types.AskQuestionEntry(
                            question=mc.question, options=opts
                        )
                    )
            ...
            if self._hook_runner and questions_list:
                ...
                (
                    ...
                    question_res,
                    _,
                ) = await self._hook_runner.dispatch_interaction(
                    turn_context=ctx,
                ...
                if question_res:
                    for orig_idx, r in zip(
                        indices_to_hook, question_res.responses
                    ):
    ...
    async def _handle_tool_confirmation_request(
        ...
        try:
            ...
            tc = types.ToolCall(
                id=_make_step_id(
                    step_update.trajectory_id, step_update.step_index
                ),
                name=action_str,
                args=args,
                canonical_path=canonical_path,
            ...
            if (
                allow
                and tc.name != DEFAULT_HOST_TOOL_NAME
                and self._hook_runner
            ):
                if op_ctx is None:
                    ctx = self._current_turn_context or hooks.TurnContext(
                ...
                self._pending_builtin_tool_calls[pending_key] = (
                    _PendingCallValue(
                        tool_call=tc,
        ...
        try:
            ...
            if self._hook_runner:
                ...
                (
                    ...
                    tc,
                    op_context,
                ) = await self._hook_runner.dispatch_pre_tool_call(
                    turn_context=ctx, tool_call=tc
                )
            ...
            if self._tool_runner:
                ...
                try:
                    ...
                    if result.error:
                        tool_error = result.exception or RuntimeError(
                            result.error
                        )
                except Exception as e:  # pylint: disable=broad-except
                    tool_error = e
                ...
                if tool_error and self._hook_runner:
                    if not op_context:
                        op_context = hooks.OperationContext(
                            self._get_turn_context()
                        )
                    (
                        recovery_res,
...
class LocalConnectionStrategy(connection.ConnectionStrategy):
        ...
        *,
        ...
        if isinstance(gemini_config, str):
            self._gemini_config = types.GeminiConfig(
                models=types.ModelConfig(
                    default=types.ModelEntry(name=gemini_config)
                )
            )
        else:
        ...
        if isinstance(system_instructions, str):
            self._system_instructions = types.TemplatedSystemInstructions(
                sections=[
                    types.SystemInstructionSection(content=system_instructions)
                ]
            )
        else:
    ...
    def _build_harness_config(self) -> localharness_pb2.HarnessConfig:
        ...
        if self._system_instructions:
            ...
            if isinstance(
                self._system_instructions, types.CustomSystemInstructions
            ):
                ...
                if self._system_instructions.identity:
                    appended.custom_identity = (
                        self._system_instructions.identity
                    )
                for sec in self._system_instructions.sections:
                    appended.appended_sections.add(
        ...
        for attempt in range(max_retries):
            try:
                ws = await websockets.connect(
                    ws_url,
                    additional_headers={
                        "x-goog-api-key": output_config.api_key
                    },
                )
                break
